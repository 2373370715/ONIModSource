...
public class Growing : StateMachineComponent<Growing.StatesInstance>, IGameObjectEffectDescriptor, IManageGrowingStates
{
	...
	public class States : GameStateMachine<States, StatesInstance, Growing>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			growing.EventTransition(GameHashes.Wilt, stalled, (StatesInstance smi) => smi.IsWilting()).EventTransition(GameHashes.CropSleep, stalled, (StatesInstance smi) => smi.IsSleeping()).EventTransition(GameHashes.ReceptacleMonitorChange, growing.planted, (StatesInstance smi) => smi.master.rm.Replanted)
				.EventTransition(GameHashes.ReceptacleMonitorChange, growing.wild, (StatesInstance smi) => !smi.master.rm.Replanted)
				.EventTransition(GameHashes.PlanterStorage, growing.planted, (StatesInstance smi) => smi.master.rm.Replanted)
	...
	public void ConsumeGrowthUnits(float units_to_consume, float unit_maturity_ratio)
	{
		...
		Debug.Assert(num <= maturity.value);
		maturity.value -= num;
		base.gameObject.Trigger(-1793167409);
	}
