...
public class ComplexFabricator : KMonoBehaviour, ISim200ms, ISim1000ms
{
	...
	{
		base.gameObject.GetComponent<KSelectable>().ToggleStatusItem(Db.Get().BuildingStatusItems.FabricatorAcceptsMutantSeeds, SaveLoader.Instance.IsDLCActiveForCurrentSave("EXPANSION1_ID") && HasRecipiesWithSeeds() && !forbidMutantSeeds);
	}
	...
	public ComplexRecipe[] GetRecipes()
	{
		if (recipe_list == null)
		{
			...
			foreach (ComplexRecipe item in recipes)
			{
				...
				{
					if (fabricator == prefabTag && SaveLoader.Instance.IsDlcListActiveForCurrentSave(item.GetDlcIds()))
					{
						list.Add(item);
					}
				}
	...
	protected virtual List<GameObject> SpawnOrderProduct(ComplexRecipe recipe)
	{
		...
		foreach (ComplexRecipe.RecipeElement recipeElement4 in ingredients)
		{
			...
			{
				GameObject gameObject2 = GameUtil.KInstantiate(Assets.GetPrefab(recipeElement4.material), Grid.SceneLayer.Ore);
			...
			case ComplexRecipe.RecipeElement.TemperatureOperation.Dehydrated:
			{
				...
				{
					GameObject gameObject3 = GameUtil.KInstantiate(Assets.GetPrefab(recipeElement4.material), Grid.SceneLayer.Ore);
			...
			case ComplexRecipe.RecipeElement.TemperatureOperation.Melted:
				if (storeProduced || recipeElement4.storeElement)
				{
					...
					outStorage.AddLiquid(ElementLoader.GetElementID(recipeElement4.material), recipeElement4.amount, temperature, 0, 0);
				}
				break;
			}
			if (list.Count <= 0)
