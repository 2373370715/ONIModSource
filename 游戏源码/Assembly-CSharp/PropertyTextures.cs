...
public class PropertyTextures : KMonoBehaviour, ISim200ms
{
	...
	protected override void OnSpawn()
	{
		...
		CameraZoomID = Shader.PropertyToID("_CameraZoomInfo");
	}
	...
	public static Vector4 CalculateClusterWorldSize()
	{
		...
		return (!DlcManager.IsPureVanilla() && (!(CameraController.Instance != null) || !CameraController.Instance.ignoreClusterFX)) ? new Vector4(worldSize.x, worldSize.y, worldOffset.x, worldOffset.y) : new Vector4(Grid.WidthInCells, Grid.HeightInCells, 0f, 0f);
	}
	...
	private void LateUpdate()
	{
		...
		Vector4 value = CalculateClusterWorldSize();
		...
		Shader.SetGlobalVector(CameraZoomID, new Vector4(CameraController.Instance.OrthographicSize, CameraController.Instance.minOrthographicSize, num, (CameraController.Instance.OrthographicSize - CameraController.Instance.minOrthographicSize) / (num - CameraController.Instance.minOrthographicSize)));
		Shader.SetGlobalVector(ClusterWorldSizeID, value);
		...
		int num2 = NextPropertyIdx++ % allTextureProperties.Count;
		TextureProperties textureProperties = allTextureProperties[num2];
		while (textureProperties.updateEveryFrame)
		{
			num2 = NextPropertyIdx++ % allTextureProperties.Count;
			textureProperties = allTextureProperties[num2];
		}
		for (int i = 0; i < allTextureProperties.Count; i++)
		{
			TextureProperties p = allTextureProperties[i];
			if (num2 == i || p.updateEveryFrame || GameUtil.IsCapturingTimeLapse())
			{
