...
public class WorldDamage : KMonoBehaviour
{
	...
	public float ApplyDamage(int cell, float amount, int src_cell, DamageType damageType, string source_name = null, string pop_text = null)
	{
		float result = 0f;
		if (Grid.Solid[cell])
		{
	...
	private IEnumerator DelayedSpawnFX(int src_cell, int dest_cell, int offset, Element elem, ushort idx, float temperature)
	{
		...
		if (offset == -1)
		{
			...
			FallingWater.instance.AddParticle(dest_cell, idx, 1f, temperature, byte.MaxValue, 0, skip_sound: true);
		}
