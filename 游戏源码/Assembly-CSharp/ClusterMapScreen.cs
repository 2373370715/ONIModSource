...
public class ClusterMapScreen : KScreen
{
	...
	public void ShowInSelectDestinationMode(ClusterDestinationSelector destination_selector)
	{
		...
		else
		{
			...
			ClusterMapHex component2 = m_cellVisByLocation[myWorldLocation].GetComponent<ClusterMapHex>();
	...
	public void SelectEntity(ClusterGridEntity entity, bool frameDelay = false)
	{
		if (entity != null)
		{
			...
			ClusterMapHex component = m_cellVisByLocation[entity.Location].GetComponent<ClusterMapHex>();
	...
	private void UpdatePaths()
	{
		...
		if (m_mode == Mode.SelectDestination && m_hoveredHex != null)
		{
			...
			if (m_selectedEntity != null)
			{
				int rangeInTiles = m_selectedEntity.GetComponent<IClusterRange>().GetRangeInTiles();
				if (num > rangeInTiles && string.IsNullOrEmpty(fail_reason))
				{
					fail_reason = string.Format(UI.CLUSTERMAP.TOOLTIP_INVALID_DESTINATION_OUT_OF_RANGE, rangeInTiles);
				}
				bool repeat = clusterDestinationSelector.GetComponent<RocketClusterDestinationSelector>().Repeat;
				m_hoveredHex.SetDestinationStatus(fail_reason, num, rangeInTiles, repeat);
			}
			else
			{
