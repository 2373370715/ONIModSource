...
public class StatusItemRenderer
{
	public struct Entry
	{
		...
		public void Render(StatusItemRenderer renderer, Vector3 camera_bl, Vector3 camera_tr, HashedString overlay, Camera camera)
		{
			...
			{
				int num2 = 0;
				spritesListedToRender.Clear();
				statusItemsToRender_Index.Clear();
				int num3 = -1;
				foreach (StatusItem statusItem3 in statusItems)
				{
					...
					Sprite sprite = statusItem3.sprite.sprite;
					if (!statusItem3.unique)
					{
						if (spritesListedToRender.Contains(sprite) || spritesListedToRender.Count >= spritesListedToRender.Capacity)
						{
							continue;
						}
						spritesListedToRender.Add(sprite);
					}
					statusItemsToRender_Index.Add(num3);
					num2++;
				}
				...
				meshBuilder.AddQuad(new Vector2(0f, 0f) + vector2, new Vector2(num4 * (float)num2, num4), z, renderer.backgroundSprite, color2);
				meshBuilder.AddQuad(new Vector2(0f, 0f), new Vector2(num4 * (float)num2 + num5, num4 + num5), z, renderer.backgroundSprite, color);
				meshBuilder.AddQuad(new Vector2(0f, 0f), new Vector2(num4 * (float)num2, num4), z, renderer.backgroundSprite, color3);
				for (int j = 0; j < statusItemsToRender_Index.Count; j++)
				{
					StatusItem statusItem = statusItems[statusItemsToRender_Index[j]];
					...
					Sprite sprite2 = statusItem.sprite.sprite;
					meshBuilder.AddQuad(new Vector2(x, 0f), new Vector2(num4, num4), z, sprite2, color);
				}
				meshBuilder.AddQuad(new Vector2(0f, 0.29f + num5), new Vector2(0.05f + num5, 0.05f + num5), z, renderer.arrowSprite, color);
