...
public class DestinationSelectPanel : KMonoBehaviour
{
	...
	public void UpdateDisplayedClusters()
	{
		...
		{
			if ((!DlcManager.FeatureClusterSpaceEnabled() || !(item.Key == "clusters/SandstoneDefault")) && item.Value.clusterCategory == (ClusterLayout.ClusterCategory)ChosenClusterCategorySetting)
			{
	...
	{
		foreach (string clusterKey in clusterKeys)
		{
			if (asteroidData[clusterKey].Layout.menuOrder == 0)
			{
				return clusterKey;
			}
		}
	...
	private void DebugCurrentSetting()
	{
		...
		case CustomGameSettings.CustomGameMode.Custom:
		{
			...
			{
				if (qualitySetting.Value.coordinate_range >= 0)
				{
