...
public class Repairable : Workable
{
	public class SMInstance : GameStateMachine<States, SMInstance, Repairable, object>.GameInstance
	{
		...
		public void DestroyStorageProxy()
		{
			...
			base.smi.master.storageProxy.DropAll(vent_gas: false, dump_liquid: false, default(Vector3), do_disease_transfer: true, list);
			GameObject gameObject = base.smi.sm.worker.Get(base.smi);
			if (gameObject != null)
			{
				foreach (GameObject item in list)
				{
					item.Trigger(580035959, gameObject.GetComponent<Worker>());
				}
			}
	...
	public class States : GameStateMachine<States, SMInstance, Repairable>
	{
		...
		private Chore CreateFetchChore(SMInstance smi)
		{
			...
			HashSet<Tag> tags = new HashSet<Tag> { GameTagExtensions.Create(component.ElementID) };
			return new FetchChore(Db.Get().ChoreTypes.RepairFetch, smi.master.storageProxy, amount, tags, FetchChore.MatchCriteria.MatchID, Tag.Invalid, null, null, run_until_complete: true, null, null, null, Operational.State.None);
		}
