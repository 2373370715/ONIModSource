...
public class BedDiagnostic : ColonyDiagnostic
{
	...
	{
		return Components.Sleepables.GetWorldItems(base.worldID).FindAll((Sleepable match) => match.GetComponent<Assignable>() != null && match.GetComponent<Clinic>() == null).Count + "/" + Components.LiveMinionIdentities.GetWorldItems(base.worldID).Count;
	}
}
