...
public class PlanScreen : KIconToggleMenu
{
	...
	{
		if (!LastSelectedBuilding.IsNullOrDestroyed() && LastSelectedBuilding.gameObject.activeInHierarchy)
		{
	...
	private void UpdateBuildingButtonList(ToggleInfo toggle_info)
	{
		...
		if (toggle != null && allBuildingToggles.Count != 0)
		{
			if (forceRefreshAllBuildings)
			{
				...
				{
					PlanBuildingToggle value = allBuildingToggles.ElementAt(i).Value;
			...
			else
			{
				for (int j = 0; j < maxToggleRefreshPerFrame; j++)
				{
					...
					PlanBuildingToggle value2 = allBuildingToggles.ElementAt(building_button_refresh_idx).Value;
					categoryPanelSizeNeedsRefresh = value2.Refresh() || categoryPanelSizeNeedsRefresh;
					value2.SwitchViewMode(useSubCategoryLayout);
		...
		foreach (KeyValuePair<string, GameObject> allSubCategoryObject in allSubCategoryObjects)
		{
			...
			if (allSubCategoryObject.Value.gameObject.activeSelf != num > 0)
			{
				allSubCategoryObject.Value.gameObject.SetActive(num > 0);
			}
		}
	...
	private void BuildButtonList()
	{
		...
		foreach (PlanInfo item in TUNING.BUILDINGS.PLANORDER)
		{
			foreach (KeyValuePair<string, string> buildingAndSubcategoryDatum in item.buildingAndSubcategoryData)
			{
				...
				{
					dictionary.Add(buildingDef.PrefabID, item.category);
					if (!dictionary2.ContainsKey(buildingAndSubcategoryDatum.Value))
					{
						dictionary2.Add(buildingAndSubcategoryDatum.Value, new List<BuildingDef>());
					}
					dictionary2[buildingAndSubcategoryDatum.Value].Add(buildingDef);
				}
			}
		}
		if (!allSubCategoryObjects.ContainsKey("default"))
		{
			allSubCategoryObjects.Add("default", Util.KInstantiateUI(subgroupPrefab, GroupsTransform.gameObject, force_active: true));
		}
		...
		foreach (KeyValuePair<string, List<BuildingDef>> item2 in dictionary2)
		{
			if (!allSubCategoryObjects.ContainsKey(item2.Key))
			{
				allSubCategoryObjects.Add(item2.Key, Util.KInstantiateUI(subgroupPrefab, GroupsTransform.gameObject, force_active: true));
			}
			if (item2.Key == "default")
			{
				allSubCategoryObjects[item2.Key].SetActive(useSubCategoryLayout);
			}
			HierarchyReferences component = allSubCategoryObjects[item2.Key].GetComponent<HierarchyReferences>();
			GameObject parent;
			if (useSubCategoryLayout)
			{
				component.GetReference<RectTransform>("Header").gameObject.SetActive(value: true);
				parent = allSubCategoryObjects[item2.Key].GetComponent<HierarchyReferences>().GetReference<GridLayoutGroup>("Grid").gameObject;
				if (Strings.TryGet("STRINGS.UI.NEWBUILDCATEGORIES." + item2.Key.ToUpper() + ".BUILDMENUTITLE", out var result))
				{
					component.GetReference<LocText>("HeaderLabel").SetText(result);
				}
			}
			else
			{
			...
			foreach (BuildingDef item3 in item2.Value)
			{
				HashedString hashedString = dictionary[item3.PrefabID];
				GameObject gameObject2 = CreateButton(item3, parent, hashedString);
				ToggleEntry toggleEntry = null;
				GetToggleEntryForCategory(hashedString, out toggleEntry);
				if (toggleEntry != null && toggleEntry.pendingResearchAttentions.Contains(item3.PrefabID))
				{
	...
	private GameObject CreateButton(BuildingDef def, GameObject parent, HashedString plan_category)
	{
		...
		{
			gameObject = allBuildingToggles[def.PrefabID].gameObject;
			planBuildingToggle = allBuildingToggles[def.PrefabID];
			planBuildingToggle.Refresh();
		}
		else
		{
			...
			planBuildingToggle.SwitchViewMode(useSubCategoryLayout);
			allBuildingToggles.Add(def.PrefabID, planBuildingToggle);
		}
