...
public class ClusterManager : KMonoBehaviour, ISaveLoadable
{
	...
	public void MigrateCritter(GameObject critter, int targetID, int prevID)
	{
		...
		Game.Instance.Trigger(1142724171, critterMigrationEvArg);
	}
	...
	public void MigrateMinion(MinionIdentity minion, int targetID, int prevID)
	{
		...
		Game.Instance.Trigger(586301400, migrationEvArg);
	}
	...
	public void UpdateRocketInteriorAudio()
	{
		...
		{
			_ = (Vector3)(worldContainer.minimumBounds + new Vector2((float)worldContainer.Width * Grid.CellSizeInMeters, (float)worldContainer.Height * Grid.CellSizeInMeters) / 2f);
