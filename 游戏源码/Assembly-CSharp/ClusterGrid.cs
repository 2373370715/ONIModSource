...
public class ClusterGrid
{
	...
	{
		return (from entity in AxialUtil.GetRing(cell, 0).SelectMany(GetEntitiesOnCell)
			where entity.Layer == entityLayer
			select entity).ToList();
	}
	...
	public bool GetVisibleUnidentifiedMeteorShowerWithinRadius(AxialI center, int radius, out ClusterMapMeteorShower.Instance result)
	{
		for (int i = 0; i <= radius; i++)
		{
			...
			{
				if (!IsValidCell(item) || !GetFOWManager().IsLocationRevealed(item))
				{
					continue;
				}
				foreach (ClusterGridEntity item2 in GetEntitiesOfLayerAtCell(item, EntityLayer.Craft))
				{
					ClusterMapMeteorShower.Instance sMI = item2.GetSMI<ClusterMapMeteorShower.Instance>();
		...
		void ExpandFrontier()
		{
			...
			foreach (AxialI item in frontier)
			{
				foreach (AxialI dIRECTION in AxialI.DIRECTIONS)
				{
					...
					if (!visited.Contains(neighbor) && IsValidCell(neighbor) && (IsCellVisible(neighbor) || destination_selector.canNavigateFogOfWar) && (!HasVisibleAsteroidAtCell(neighbor) || !(neighbor != start) || !(neighbor != end)) && (!dodgeHiddenAsteroids || !(Instance.GetAsteroidAtCell(neighbor) != null) || Instance.GetAsteroidAtCell(neighbor).IsVisibleInFOW == ClusterRevealLevel.Visible || !(neighbor != start) || !(neighbor != end)))
					{
						buffer.Add(neighbor);
