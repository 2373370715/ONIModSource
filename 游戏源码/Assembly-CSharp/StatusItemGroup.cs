...
public class StatusItemGroup
{
	...
	{
		public static Entry EmptyEntry = new Entry
		{
			id = Guid.Empty
		};
	...
	public bool HasStatusItemID(string status_item_id)
	{
		for (int i = 0; i < items.Count; i++)
		{
			if (items[i].item.Id == status_item_id)
			{
				return true;
			}
		}
	...
	public Guid RemoveStatusItem(StatusItem status_item, bool immediate = false)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			if (items[i].item.Id == status_item.Id)
			{
				...
				RemoveStatusItemInternal(id, i, immediate);
				return id;
			}
		}
		return Guid.Empty;
	...
	public Guid RemoveStatusItem(Guid guid, bool immediate = false)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			if (items[i].id == guid)
			{
				RemoveStatusItemInternal(guid, i, immediate);
				return guid;
			}
		}
		return Guid.Empty;
