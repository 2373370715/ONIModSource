...
public class TemplateContainer
{
	...
	public class Info
	{
		...
		{
			return new RectInt(position.x + (int)min.x - padding, position.y + (int)min.y - padding, (int)size.x + padding * 2, (int)size.y + padding * 2);
		}
	}
	...
	{
		if ((info.min - new Vector2f(0, 0)).sqrMagnitude <= 1E-06f)
		{
			RefreshInfo();
		}
		return info.GetBounds(position, padding);
	}
