...
public class SimDebugView : KMonoBehaviour
{
	...
	private static float[] relativeTemperatureColorIntervals = new float[7] { 0.4f, 0.05f, 0.05f, 0.05f, 0.05f, 0.2f, 0.2f };

	private static float[] absoluteTemperatureColorIntervals = new float[8] { 273.15f, 10f, 10f, 10f, 7f, 63f, 1700f, 10000f };
	...
	{
		float num = user_temperatureThresholds[0];
		float num2 = user_temperatureThresholds[1];
		float num3 = num2 - num;
		if (actualTemperature < num)
		{
			return GlobalAssets.Instance.colorSet.GetColorByName(temperatureThresholds[0].colorName);
		}
		if (actualTemperature > num2)
		{
			return GlobalAssets.Instance.colorSet.GetColorByName(temperatureThresholds[temperatureThresholds.Length - 1].colorName);
		}
		int num4 = 0;
		float t = 0f;
		...
		{
			float num6 = num;
			for (int j = 0; j < absoluteTemperatureColorIntervals.Length; j++)
			{
				if (actualTemperature < num6 + absoluteTemperatureColorIntervals[j])
				{
					num4 = j;
					break;
				}
				num6 += absoluteTemperatureColorIntervals[j];
			}
			t = (actualTemperature - num6) / absoluteTemperatureColorIntervals[num4];
			break;
		}
		case Game.TemperatureOverlayModes.RelativeTemperature:
		{
			float num5 = num;
			for (int i = 0; i < relativeTemperatureColorIntervals.Length; i++)
			{
				if (actualTemperature < num5 + relativeTemperatureColorIntervals[i] * num3)
				{
					num4 = i;
					break;
				}
				num5 += relativeTemperatureColorIntervals[i] * num3;
			}
			t = (actualTemperature - num5) / (relativeTemperatureColorIntervals[num4] * num3);
			break;
		}
		}
		return Color.Lerp(GlobalAssets.Instance.colorSet.GetColorByName(temperatureThresholds[num4].colorName), GlobalAssets.Instance.colorSet.GetColorByName(temperatureThresholds[num4 + 1].colorName), t);
	}
