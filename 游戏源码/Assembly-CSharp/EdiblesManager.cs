...
public class EdiblesManager : KMonoBehaviour
{
	...
	{
		return s_allFoodTypes.Where((FoodInfo x) => DlcManager.IsContentSubscribed(x.DlcId)).ToList();
	}
	...
	public static List<FoodInfo> GetAllFoodTypes()
	{
		...
		return s_allFoodTypes.Where((FoodInfo x) => SaveLoader.Instance.IsDLCActiveForCurrentSave(x.DlcId)).ToList();
	}
