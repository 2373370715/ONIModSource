...
public class StatusItem : Resource
{
	...
	private void SetIcon(string icon, IconType icon_type, NotificationType notification_type, bool allow_multiples, HashedString render_overlay, bool show_world_icon = true, int status_overlays = 129022, Func<string, object, string> resolve_string_callback = null)
	{
		switch (icon_type)
		{
		case IconType.Info:
	...
	{
		SetIcon(icon, icon_type, notification_type, allow_multiples, render_overlay, showWorldIcon, status_overlays, resolve_string_callback);
	}
	...
	{
		if (resolveStringCallback != null && (data != null || resolveStringCallback_shouldStillCallIfDataIsNull))
		{
	...
	private string ResolveTooltip(string str, object data)
	{
		...
		{
			if (resolveStringCallback_shouldStillCallIfDataIsNull && resolveStringCallback != null)
			{
				return resolveStringCallback(str, data);
			}
			if (resolveTooltipCallback_shouldStillCallIfDataIsNull && resolveTooltipCallback != null)
			{
				return resolveTooltipCallback(str, data);
			}
		}
