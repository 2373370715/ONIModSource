		...
		else
		{
			...
			foreach (TechItem unlockedItem in targetTech.unlockedItems)
			{
				if (SaveLoader.Instance.IsDlcListActiveForCurrentSave(unlockedItem.dlcIds))
				{
					...
					bool flag = !DlcManager.IsValidForVanilla(unlockedItem.dlcIds);
					reference.gameObject.SetActive(flag);
					if (flag)
					{
						reference.color = DlcManager.GetDlcBannerColor(unlockedItem.dlcIds[0]);
					}
					string text2 = $"{unlockedItem.Name}\n{unlockedItem.description}";
					if (!DlcManager.IsValidForVanilla(unlockedItem.dlcIds))
					{
						text2 += string.Format(RESEARCH.MESSAGING.DLC.DLC_CONTENT, DlcManager.GetDlcTitle(unlockedItem.dlcIds[0]));
					}
