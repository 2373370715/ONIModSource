...
public class ManualDeliveryKG : KMonoBehaviour, ISim1000ms
{
	...
	private float MassStoredPerUnit => storage.GetMassAvailable(requestedItemTag) / MassPerUnit;
	...
	public void RequestDelivery()
	{
		if (fetchList == null)
		{
			float massStoredPerUnit = MassStoredPerUnit;
			if (massStoredPerUnit < capacity)
			{
				CreateFetchChore(massStoredPerUnit);
			}
		}
	}
	...
	private void UpdateFetchList()
	{
		...
		else if (fetchList == null && MassStoredPerUnit < refillMass)
		{
			RequestDelivery();
		}
	}
