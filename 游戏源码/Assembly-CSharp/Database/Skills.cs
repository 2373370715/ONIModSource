...
public class Skills : ResourceSet<Skill>
{
		...
		: base("Skills", parent)
	{
		...
		Pyrotechnics = AddSkill(new Skill("Pyrotechnics", DUPLICANTS.ROLES.PYROTECHNIC.NAME, DUPLICANTS.ROLES.PYROTECHNIC.DESCRIPTION, "", 2, "hat_role_technicals2", "skillbadge_role_technicals2", Db.Get().SkillGroups.Basekeeping.Id, new List<SkillPerk> { Db.Get().SkillPerks.CanMakeMissiles }, new List<string> { Basekeeping2.Id }));
		if (DlcManager.IsExpansion1Active())
		{
