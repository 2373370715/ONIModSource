...
public class PlantMutations : ResourceSet<PlantMutation>
{
	...
	{
		moderatelyLoose = AddPlantMutation("moderatelyLoose").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.WiltTempRangeMod, 0.5f, multiplier: true).AttributeModifier(Db.Get().PlantAttributes.YieldAmount, -0.25f, multiplier: true)
			.AttributeModifier(Db.Get().PlantAttributes.FertilizerUsageMod, -0.5f, multiplier: true)
			.VisualTint(-0.4f, -0.4f, -0.4f);
		moderatelyTight = AddPlantMutation("moderatelyTight").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.WiltTempRangeMod, -0.5f, multiplier: true).AttributeModifier(Db.Get().PlantAttributes.YieldAmount, 0.5f, multiplier: true)
			.VisualTint(0.2f, 0.2f, 0.2f);
		extremelyTight = AddPlantMutation("extremelyTight").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.WiltTempRangeMod, -0.8f, multiplier: true).AttributeModifier(Db.Get().PlantAttributes.YieldAmount, 1f, multiplier: true)
			.VisualTint(0.3f, 0.3f, 0.3f)
			.VisualBGFX("mutate_glow_fx_kanim");
		bonusLice = AddPlantMutation("bonusLice").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.FertilizerUsageMod, 0.25f, multiplier: true).BonusCrop("BasicPlantFood", 1f)
		...
		sunnySpeed = AddPlantMutation("sunnySpeed").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.MinLightLux, 1000f).AttributeModifier(Db.Get().Amounts.Maturity.maxAttribute, -0.5f, multiplier: true)
		...
		slowBurn = AddPlantMutation("slowBurn").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.FertilizerUsageMod, -0.9f, multiplier: true).AttributeModifier(Db.Get().Amounts.Maturity.maxAttribute, 3.5f, multiplier: true)
			.VisualTint(-0.3f, -0.3f, -0.5f);
		blooms = AddPlantMutation("blooms").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().BuildingAttributes.Decor, 20f).VisualSymbolOverride("snapTo_mutate1", "mutate_snaps_kanim", "blossom_mutate1")
		...
		loadedWithFruit = AddPlantMutation("loadedWithFruit").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.YieldAmount, 1f, multiplier: true).AttributeModifier(Db.Get().PlantAttributes.HarvestTime, 4f, multiplier: true)
		...
		rottenHeaps = AddPlantMutation("rottenHeaps").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().Amounts.Maturity.maxAttribute, -0.75f, multiplier: true).AttributeModifier(Db.Get().PlantAttributes.FertilizerUsageMod, 0.5f, multiplier: true)
		...
		heavyFruit = AddPlantMutation("heavyFruit").AttributeModifier(Db.Get().PlantAttributes.MinRadiationThreshold, 250f).AttributeModifier(Db.Get().PlantAttributes.FertilizerUsageMod, 0.25f, multiplier: true).ForceSelfHarvestOnGrown()
