...
public class CreatureStatusItems : StatusItems
{
	...
	private void CreateStatusItems()
	{
		...
		Scalding.resolveTooltipCallback = delegate(string str, object data)
		{
			...
			str = str.Replace("{ExternalTemperature}", GameUtil.GetFormattedTemperature(averageExternalTemperature2));
		...
		{
			SpaceTreePlant.Instance instance19 = (SpaceTreePlant.Instance)data;
			str = str.Replace("{0}", GameUtil.GetFormattedPercent(instance19.CurrentProductionProgress * 100f));
			return str;
		};
		ProducingSugarWater.resolveTooltipCallback = delegate(string str, object data)
		{
			SpaceTreePlant.Instance instance18 = (SpaceTreePlant.Instance)data;
			PlantBranchGrower.Instance sMI3 = instance18.GetSMI<PlantBranchGrower.Instance>();
			for (int j = 0; j < instance18.def.OptimalAmountOfBranches; j++)
			{
				string text3 = CREATURES.STATUSITEMS.PRODUCINGSUGARWATER.BRANCH_LINE_MISSING;
				string newValue8 = SpaceTreeBranchConfig.BRANCH_NAMES[j];
				...
				if (branch2 != null)
				{
					...
					if (sMI4 != null && !sMI4.isMasterNull)
					{
						...
						{
							string formattedPercent3 = GameUtil.GetFormattedPercent(sMI4.Productivity * 100f);
							text3 = CREATURES.STATUSITEMS.PRODUCINGSUGARWATER.BRANCH_LINE;
							text3 = text3.Replace("{1}", formattedPercent3);
						}
						else
						{
							string formattedPercent4 = GameUtil.GetFormattedPercent(sMI4.GetcurrentGrowthPercentage() * 100f);
							text3 = CREATURES.STATUSITEMS.PRODUCINGSUGARWATER.BRANCH_LINE_GROWING;
							text3 = text3.Replace("{1}", formattedPercent4);
						}
					}
				}
				text3 = text3.Replace("{0}", newValue8);
				string oldValue2 = "{BRANCH_" + j + "}";
				str = str.Replace(oldValue2, text3);
			}
			str = str.Replace("{0}", GameUtil.GetFormattedMass(instance18.GetProductionSpeed() * 20f / instance18.OptimalProductionDuration, GameUtil.TimeSlice.PerSecond));
			str = str.Replace("{1}", instance18.def.OptimalAmountOfBranches.ToString());
		...
		{
			SpaceTreePlant.Instance instance17 = (SpaceTreePlant.Instance)data;
			str = str.Replace("{0}", GameUtil.GetFormattedPercent(instance17.CurrentProductionProgress * 100f));
			return str;
		};
		SugarWaterProductionPaused.resolveTooltipCallback = delegate(string str, object data)
		{
			SpaceTreePlant.Instance instance16 = (SpaceTreePlant.Instance)data;
			PlantBranchGrower.Instance sMI = instance16.GetSMI<PlantBranchGrower.Instance>();
			for (int i = 0; i < instance16.def.OptimalAmountOfBranches; i++)
			{
				string text2 = CREATURES.STATUSITEMS.SUGARWATERPRODUCTIONPAUSED.BRANCH_LINE_MISSING;
				string newValue7 = SpaceTreeBranchConfig.BRANCH_NAMES[i];
				...
				if (branch != null)
				{
					...
					if (sMI2 != null && !sMI2.isMasterNull)
					{
						if (sMI2.IsBranchFullyGrown)
						{
							...
							text2 = CREATURES.STATUSITEMS.SUGARWATERPRODUCTIONPAUSED.BRANCH_LINE;
							text2 = text2.Replace("{1}", formattedPercent);
						}
						else
						{
							string formattedPercent2 = GameUtil.GetFormattedPercent(sMI2.GetcurrentGrowthPercentage() * 100f);
							text2 = CREATURES.STATUSITEMS.SUGARWATERPRODUCTIONPAUSED.BRANCH_LINE_GROWING;
							text2 = text2.Replace("{1}", formattedPercent2);
						}
					}
				}
				text2 = text2.Replace("{0}", newValue7);
				string oldValue = "{BRANCH_" + i + "}";
				str = str.Replace(oldValue, text2);
			}
			str = str.Replace("{0}", instance16.def.OptimalAmountOfBranches.ToString());
		...
		{
			SpaceTreePlant.Instance instance15 = (SpaceTreePlant.Instance)data;
			str = str.Replace("{0}", GameUtil.GetFormattedPercent(instance15.CurrentProductionProgress * 100f));
			return str;
		};
		...
		{
			SpaceTreeBranch.Instance instance14 = (SpaceTreeBranch.Instance)data;
			str = str.Replace("{0}", GameUtil.GetFormattedPercent(instance14.Productivity * 100f));
			return str;
		};
		SpaceTreeBranchLightStatus.resolveTooltipCallback = delegate(string str, object data)
		{
			SpaceTreeBranch.Instance instance13 = (SpaceTreeBranch.Instance)data;
			str = str.Replace("{0}", GameUtil.GetFormattedLux(instance13.def.OPTIMAL_LUX_LEVELS));
			str = str.Replace("{1}", GameUtil.GetFormattedLux(instance13.CurrentAmountOfLux));
			return str;
		};
		...
		{
			CropSleepingMonitor.Instance instance12 = (CropSleepingMonitor.Instance)data;
			return str.Replace("{REASON}", instance12.def.prefersDarkness ? CREATURES.STATUSITEMS.CROP_SLEEPING.REASON_TOO_BRIGHT : CREATURES.STATUSITEMS.CROP_SLEEPING.REASON_TOO_DARK);
		};
		CropSleeping.resolveTooltipCallback = delegate(string str, object data)
		{
			CropSleepingMonitor.Instance instance11 = (CropSleepingMonitor.Instance)data;
			AttributeInstance attributeInstance = Db.Get().PlantAttributes.MinLightLux.Lookup(instance11.gameObject);
		...
		Fresh.resolveTooltipCallback = delegate(string str, object data)
		{
			...
			return str.Replace("{RotTooltip}", instance9.GetToolTip());
		};
		...
		{
			Rottable.Instance instance8 = (Rottable.Instance)data;
			return str.Replace("{RotPercentage}", "(" + Util.FormatWholeNumber(instance8.RotConstitutionPercentage * 100f) + "%)");
		};
		Stale.resolveTooltipCallback = delegate(string str, object data)
		{
			Rottable.Instance instance7 = (Rottable.Instance)data;
			return str.Replace("{RotTooltip}", instance7.GetToolTip());
		};
		...
		{
			AgeMonitor.Instance instance6 = (AgeMonitor.Instance)data;
			return str.Replace("{TimeUntilDeath}", GameUtil.GetFormattedCycles(instance6.CyclesUntilDeath * 600f));
		};
		...
		{
			Diet diet2 = ((CreatureCalorieMonitor.Instance)data).stomach.diet;
		...
		{
			ElementGrowthMonitor.Instance instance5 = (ElementGrowthMonitor.Instance)data;
			StringBuilder stringBuilder = new StringBuilder(str, str.Length * 2);
			stringBuilder.Replace("{templo}", GameUtil.GetFormattedTemperature(instance5.def.minTemperature));
			stringBuilder.Replace("{temphi}", GameUtil.GetFormattedTemperature(instance5.def.maxTemperature));
			if (instance5.lastConsumedTemperature > 0f)
			{
				stringBuilder.Append("\n\n");
				stringBuilder.Append(CREATURES.STATUSITEMS.ELEMENT_GROWTH_GROWING.PREFERRED_TEMP);
				stringBuilder.Replace("{element}", ElementLoader.FindElementByHash(instance5.lastConsumedElement).name);
				stringBuilder.Replace("{temperature}", GameUtil.GetFormattedTemperature(instance5.lastConsumedTemperature));
			}
		...
		{
			ElementGrowthMonitor.Instance instance4 = (ElementGrowthMonitor.Instance)data;
			string newValue = ((instance4.lastConsumedTemperature < instance4.def.minTemperature) ? CREATURES.STATUSITEMS.ELEMENT_GROWTH_STUNTED.TOO_COLD : CREATURES.STATUSITEMS.ELEMENT_GROWTH_STUNTED.TOO_HOT);
		...
		{
			MilkProductionMonitor.Instance instance3 = (MilkProductionMonitor.Instance)data;
			str = str.Replace("{amount}", GameUtil.GetFormattedMass(instance3.MilkPercentage));
			return str;
		};
		...
		{
			CritterTemperatureMonitor.Instance instance2 = (CritterTemperatureMonitor.Instance)obj;
			return string.Format(str, GameUtil.GetFormattedTemperature(instance2.GetTemperatureInternal()), GameUtil.GetFormattedTemperature(instance2.def.temperatureColdUncomfortable), GameUtil.GetFormattedTemperature(instance2.def.temperatureHotUncomfortable), Effect.CreateTooltip(instance2.sm.uncomfortableEffect, showDuration: false));
		};
