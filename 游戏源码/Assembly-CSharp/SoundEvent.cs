...
public class SoundEvent : AnimEvent
{
	...
	{
		return KFMOD.BeginOneShot(event_ref, GetCameraScaledPosition(pos, objectIsSelectedAndVisible), volume);
	}
	...
	{
		return BeginOneShot(RuntimeManager.PathToEventReference(ev), pos, volume);
	}
	...
	public static bool PlayOneShot(EventReference event_ref, Vector3 sound_pos, float volume = 1f)
	{
		...
		if (!event_ref.IsNull)
		{
			FMOD.Studio.EventInstance instance = BeginOneShot(event_ref, sound_pos, volume);
			if (instance.isValid())
			{
	...
	{
		return PlayOneShot(RuntimeManager.PathToEventReference(sound), sound_pos, volume);
	}
	...
	{
		if (sound != null && Camera.main != null && Camera.main.orthographicSize > AudioMixer.LOW_PRIORITY_CUTOFF_DISTANCE && !AudioMixer.instance.activeNIS && GlobalAssets.IsLowPriority(sound))
		{
			return true;
		}
		return false;
