...
public class OfflineWorldGen : KMonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!debug && currentConvertedCurrentStage.Hash == UI.WORLDGEN.COMPLETE.key.Hash && currentPercent >= 1f && cluster.IsGenerationComplete)
		{
	...
	private void DoWorldGenInitialize()
	{
		...
		seed = CustomGameSettings.Instance.GetCurrentWorldgenSeed();
		text = CustomGameSettings.Instance.GetCurrentQualitySetting(CustomGameSettingConfigs.ClusterLayout).id;
		List<string> list = new List<string>();
		foreach (string currentStory in CustomGameSettings.Instance.GetCurrentStories())
		{
			list.Add(Db.Get().Stories.Get(currentStory).worldgenStoryTraitKey);
		}
		cluster = new Cluster(text, seed, list, assertMissingTraits: true, skipWorldTraits: false);
		cluster.ShouldSkipWorldCallback = shouldSkipWorldCallback;
		cluster.Generate(UpdateProgress, OnError, seed, seed, seed, seed);
	}
