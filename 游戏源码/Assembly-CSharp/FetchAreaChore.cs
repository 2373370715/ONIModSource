...
public class FetchAreaChore : Chore<FetchAreaChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, FetchAreaChore, object>.GameInstance
	{
		...
		public void Begin(Precondition.Context context)
		{
			...
			GameScenePartitioner.Instance.GatherEntries(x2, y2, num3, num3, GameScenePartitioner.Instance.storedPickupablesLayer, pooledList3);
			...
			for (int i = 0; i < pooledList3.Count; i++)
			{
				...
				if (!kPrefabID.HasTag(GameTags.StoredPrivate) && !(kPrefabID.PrefabTag != prefabTag) && !(pickupable2.UnreservedAmount <= 0f) && (rootChore.criteria != 0 || rootChore.tags.Contains(kPrefabID.PrefabTag)) && (rootChore.criteria != FetchChore.MatchCriteria.MatchTags || kPrefabID.HasTag(rootChore.tagsFirst)) && (!rootChore.requiredTag.IsValid || kPrefabID.HasTag(rootChore.requiredTag)) && !kPrefabID.HasAnyTags(rootChore.forbiddenTags) && !list.Contains(pickupable2) && rootContext.consumerState.consumer.CanReach(pickupable2) && !kPrefabID.HasTag(GameTags.MarkedForMove))
				{
		...
		public void SetupDelivery()
		{
			...
			if (s_transientDeliveryTags.Contains(nextDelivery.chore.requiredTag))
			{
		...
		public void ReservePickupables()
		{
			...
			foreach (Pickupable fetchable in fetchables)
			{
				...
				if (!fetchable.KPrefabID.HasTag(GameTags.MarkedForMove))
				{
	...
	public class States : GameStateMachine<States, StatesInstance, FetchAreaChore>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			fetching.movetopickupable.InitializeStates(fetcher, fetchTarget, fetching.pickup, fetching.fetchfail, null, NavigationTactics.ReduceTravelDistance).Target(fetchTarget).EventHandlerTransition(GameHashes.TagsChanged, fetching.fetchfail, (StatesInstance smi, object obj) => smi.RootChore_ValidateRequiredTagOnTagChange && smi.RootChore_RequiredTag.IsValid && !fetchTarget.Get(smi).HasTag(smi.RootChore_RequiredTag))
				.Target(fetcher);
