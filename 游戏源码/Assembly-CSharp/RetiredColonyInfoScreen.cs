...
public class RetiredColonyInfoScreen : KModalScreen
{
	...
	{
		if (DlcManager.HasAnyContentSubscribed(dlcid))
		{
			if (SaveLoader.Instance != null)
			{
				if (clusterTag != null && !CustomGameSettings.Instance.GetCurrentClusterLayout().clusterTags.Contains(clusterTag))
				{
					return false;
				}
				return SaveLoader.Instance.IsDlcListActiveForCurrentSave(dlcid);
			}
			return true;
		}
		return false;
	...
	private void PopulateAchievements()
	{
		...
		{
			if (!IsAchievementValidForDLCContext(resource.dlcIds, null))
			{
				continue;
			}
			...
			if (DlcManager.IsDlcId(resource.dlcIdFrom))
			{
				...
				reference.sprite = Assets.GetSprite(DlcManager.GetDlcBanner(resource.dlcIdFrom));
	...
	private void UpdateAchievementData(RetiredColonyData data, string[] newlyAchieved = null)
	{
		...
		foreach (KeyValuePair<string, GameObject> achievementEntry in achievementEntries)
		{
			...
			ColonyAchievement colonyAchievement = Db.Get().ColonyAchievements.TryGet(achievementEntry.Key);
			if (colonyAchievement != null && !IsAchievementValidForDLCContext(colonyAchievement.dlcIds, colonyAchievement.clusterTag))
			{
	...
	private void CorrectTimelapseImageSize(Sprite sprite)
	{
		...
		float minWidth = (component.preferredWidth = fittedSize.x);
		component.minWidth = minWidth;
		minWidth = (component.preferredHeight = fittedSize.y);
		component.minHeight = minWidth;
	}
