...
public class AchievementWidget : KMonoBehaviour
{
	...
	private void ConfigureToolTip(ToolTip tooltip, string status)
	{
		...
		{
			tooltip.AddMultiStringTooltip(string.Format(COLONY_ACHIEVEMENTS.DLC_ACHIEVEMENT, DlcManager.GetDlcTitle(dlcIdFrom)), null);
		}
	}
	...
	private void ShowRocketMoraleRequirement(bool success, ColonyAchievementRequirement req)
	{
		...
		foreach (KeyValuePair<int, int> item in SaveGame.Instance.ColonyAchievementTracker.cyclesRocketDupeMoraleAboveRequirement)
		{
