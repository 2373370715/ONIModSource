...
public class StoredMinionIdentity : KMonoBehaviour, ISaveLoadable, IAssignableIdentity, IListableOption, IPersonalPriorityManager
{
	...
	private void OnDeserializedMethod()
	{
		...
		{
			bodyData = Accessorizer.UpdateAccessorySlots(nameStringKey, ref accessories);
		}
		if (clothingItems.Count > 0)
		{
			customClothingItems[ClothingOutfitUtility.OutfitType.Clothing] = new List<ResourceRef<ClothingItemResource>>(clothingItems);
			clothingItems.Clear();
		}
		List<ResourceRef<Accessory>> list = accessories.FindAll((ResourceRef<Accessory> acc) => acc.Get() == null);
		if (list.Count > 0)
		{
			List<ClothingItemResource> list2 = new List<ClothingItemResource>();
			foreach (ResourceRef<Accessory> item3 in list)
			{
				ClothingItemResource clothingItemResource = Db.Get().Permits.ClothingItems.TryResolveAccessoryResource(item3.Guid);
				if (clothingItemResource != null && !list2.Contains(clothingItemResource))
				{
					list2.Add(clothingItemResource);
					customClothingItems[ClothingOutfitUtility.OutfitType.Clothing].Add(new ResourceRef<ClothingItemResource>(clothingItemResource));
				}
			}
			bodyData = Accessorizer.UpdateAccessorySlots(nameStringKey, ref accessories);
		}
	...
	private void OnDeserializeModifiers()
	{
		...
		{
			Klei.AI.Attribute attribute = Db.Get().Attributes.TryGet(savedAttributeValue.Key);
			if (attribute == null)
			{
	...
	{
		if (customClothingItems.ContainsKey(outfitType))
		{
			string[] array = new string[customClothingItems[outfitType].Count];
			for (int i = 0; i < customClothingItems[outfitType].Count; i++)
			{
				array[i] = customClothingItems[outfitType][i].Get().Id;
			}
			return array;
		}
		return null;
	...
	{
		return !forbiddenTagSet.Contains(consumable);
	}
