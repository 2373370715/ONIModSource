...
public class MultiToggle : KMonoBehaviour, IPointerClickHandler, IEventSystemHandler, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
{
	...
	{
		if (!stateDirty && new_state_index == state)
		{
			return;
		}
		stateDirty = false;
	...
	public virtual void OnPointerDown(PointerEventData eventData)
	{
		...
		{
			ToggleState toggleState = states[state];
			...
			else if (on_click_override_sound_path != "" && has_sound_parameter)
			{
				KFMOD.PlayUISoundWithParameter(GlobalAssets.GetSound("General_Item_Click"), toggleState.sound_parameter_name, toggleState.sound_parameter_value);
				KFMOD.PlayUISoundWithParameter(GlobalAssets.GetSound(on_click_override_sound_path), toggleState.sound_parameter_name, toggleState.sound_parameter_value);
			}
			else
			{
				KFMOD.PlayUISound(GlobalAssets.GetSound(on_click_override_sound_path));
			}
		}
	}
