...
public class ClusterTraveler : KMonoBehaviour, ISim200ms
{
	...
	public void Sim200ms(float dt)
	{
		...
		if (flag)
		{
			if (flag2)
			{
				...
				_ = 0;
			if (!flag2 || CurrentPath.Count > 1 || !quickTravelToAsteroidIfInOrbit)
			{
				...
				if (m_movePotential >= 600f)
				{
					...
					{
						Debug.Assert(ClusterGrid.Instance.GetVisibleEntityOfLayerAtCell(m_clusterGridEntity.Location, EntityLayer.Asteroid) == null || (flag2 && CurrentPath.Count == 0), $"Somehow this clustercraft pathed through an asteroid at {m_clusterGridEntity.Location}");
						m_movePotential -= 600f;
						if (onTravelCB != null)
						{
							onTravelCB();
	...
	private bool HasCurrentPathChanged(out string reason, out List<AxialI> updatedPath)
	{
		...
		m_isPathDirty = false;
		updatedPath = ClusterGrid.Instance.GetPath(m_clusterGridEntity.Location, m_cachedPathDestination, m_destinationSelector, out reason, m_destinationSelector.dodgesHiddenAsteroids);
		if (updatedPath == null)
		{
			return true;
		}
