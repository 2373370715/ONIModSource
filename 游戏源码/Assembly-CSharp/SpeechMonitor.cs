...
public class SpeechMonitor : GameStateMachine<SpeechMonitor, SpeechMonitor.Instance, IStateMachineTarget, SpeechMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void DrawMouth()
		{
			...
			if (!(firstFrameElement.symbol == HashedString.Invalid))
			{
				...
				KAnim.Build.SymbolFrameInstance symbol_frame_instance = KAnimBatchManager.Instance().GetBatchGroupData(symbol.build.batchTag).symbolFrameInstances[symbol.firstFrameIdx + firstFrameElement.frame];
	...
	private static bool CanOverrideHead(KBatchedAnimController kbac)
	{
		...
		if (KAnimBatchManager.Instance().GetBatchGroupData(currentAnim.animFile.animBatchTag).TryGetFrame(currentFrameIndex, out var frame) && frame.hasHead)
		{
			result = true;
		}
		return result;
	...
	public static KAnim.Anim.FrameElement GetFirstFrameElement(KBatchedAnimController controller)
	{
		...
		if (!controller.GetBatch().group.data.TryGetFrame(currentFrameIndex, out var frame))
		{
			return result;
		}
