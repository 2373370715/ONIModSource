...
public class ReceptacleSideScreen : SideScreenContent, IRender1000ms
{
	...
	public void Initialize(SingleEntityReceptacle target)
	{
		...
		entityToggles.Clear();
		foreach (Tag possibleDepositObjectTag in targetReceptacle.possibleDepositObjectTags)
		{
			...
			{
				if (!targetReceptacle.IsValidEntity(item))
				{
					num--;
					continue;
				}
			...
			Debug.Assert(list.Count == num, "Not all entities in this receptacle implement IHasSortOrder!");
			...
			foreach (IHasSortOrder item2 in list)
			{
				...
				string entityName = GetEntityName(gameObject.PrefabID());
				...
				if (component3 != null)
				{
					component3.SetSimpleTooltip(GetEntityTooltip(gameObject.PrefabID()));
				}
