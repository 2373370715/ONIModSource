...
public class SolidTransferArm : StateMachineComponent<SolidTransferArm.SMInstance>, ISim1000ms, IRenderEveryTick
{
	...
	private struct BatchUpdateTask : IWorkItem<BatchUpdateContext>
	{
		...
		public BatchUpdateTask(int start, int end)
		{
			...
			pickupable_entries = ListPool<ScenePartitionerEntry, SolidTransferArm>.Allocate();
		}
		...
		public void Run(BatchUpdateContext context)
		{
			...
			{
				context.refreshed_reachable_cells[i] = context.solid_transfer_arms[i].AsyncUpdate(context.cells[i], reachable_cells_workspace, pickupable_entries, context.game_objects[i]);
			}
		}
	...
	private bool AsyncUpdate(int cell, HashSet<int> workspace, List<ScenePartitionerEntry> entries, GameObject game_object)
	{
		...
		pickupables.Clear();
		entries.Clear();
		GameScenePartitioner.Instance.UnsafeReadonlyGatherEntries(x - pickupRange, y - pickupRange, 2 * pickupRange + 1, 2 * pickupRange + 1, GameScenePartitioner.Instance.pickupablesLayer, entries);
		GameScenePartitioner.Instance.UnsafeReadonlyGatherEntries(x - pickupRange, y - pickupRange, 2 * pickupRange + 1, 2 * pickupRange + 1, GameScenePartitioner.Instance.storedPickupablesLayer, entries);
		foreach (ScenePartitionerEntry entry in entries)
		{
			Pickupable pickupable = entry.obj as Pickupable;
			if (Grid.GetCellRange(cell, pickupable.cachedCell) <= pickupRange && IsPickupableRelevantToMyInterests(pickupable.KPrefabID, pickupable.cachedCell) && pickupable.CouldBePickedUpByTransferArm(game_object))
			{
