...
public class SleepChoreMonitor : GameStateMachine<SleepChoreMonitor, SleepChoreMonitor.Instance>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		passingout.EventTransition(GameHashes.AssignablesChanged, checkforbed).EventHandlerTransition(GameHashes.AssignableReachabilityChanged, checkforbed, (Instance smi, object data) => smi.IsBedReachable()).ToggleChore(CreatePassingOutChore, satisfied, satisfied);
		passingout_bedassigned.ParamTransition(bed, checkforbed, (Instance smi, GameObject p) => p == null).EventTransition(GameHashes.AssignablesChanged, checkforbed).EventTransition(GameHashes.AssignableReachabilityChanged, checkforbed, (Instance smi) => !smi.IsBedReachable())
			.ToggleChore(CreateExhaustedSleepChore, satisfied, satisfied);
		sleeponfloor.EventTransition(GameHashes.AssignablesChanged, checkforbed).EventHandlerTransition(GameHashes.AssignableReachabilityChanged, checkforbed, (Instance smi, object data) => smi.IsBedReachable()).ToggleChore(CreateSleepOnFloorChore, satisfied, satisfied);
	...
	{
		return new SleepChore(Db.Get().ChoreTypes.Sleep, smi.master, bed.Get(smi), bedIsLocator: false, isInterruptable: true, new StatusItem[1] { Db.Get().DuplicantStatusItems.SleepingExhausted });
	}
}
