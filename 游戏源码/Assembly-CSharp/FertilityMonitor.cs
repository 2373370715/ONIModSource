...
public class FertilityMonitor : GameStateMachine<FertilityMonitor, FertilityMonitor.Instance, IStateMachineTarget, FertilityMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void LayEgg()
		{
			...
			Tag tag = EggBreedingRoll(breedingChances);
			if (GenericGameSettings.instance.acceleratedLifecycle)
			{
				float num = 0f;
			...
			Debug.Assert(tag != Tag.Invalid, "Didn't pick an egg to lay. Weights weren't normalized?");
	...
	{
		if (smi.HasTag(GameTags.Creatures.PausedReproduction))
		{
			return false;
		}
	...
	public static Tag EggBreedingRoll(List<BreedingChance> breedingChances, bool excludeOriginalCreature = false)
	{
