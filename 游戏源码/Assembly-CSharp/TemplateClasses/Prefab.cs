...
public class Prefab
{
	...
	public Prefab(string _id, Type _type, int loc_x, int loc_y, SimHashes _element, float _temperature = -1f, float _units = 1f, string _disease = null, int _disease_count = 0, Orientation _rotation = Orientation.Neutral, template_amount_value[] _amount_values = null, template_amount_value[] _other_values = null, int _connections = 0, string facadeIdId = null)
	{
		id = _id;
		type = _type;
		location_x = loc_x;
	...
	{
		Prefab prefab = new Prefab(id, type, offset.x + location_x, offset.y + location_y, element, temperature, units, diseaseName, diseaseCount, rotationOrientation, amounts, other_values, connections, facadeId);
		if (rottable != null)
		{
