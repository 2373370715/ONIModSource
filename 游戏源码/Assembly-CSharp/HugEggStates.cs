...
public class HugEggStates : GameStateMachine<HugEggStates, HugEggStates.Instance, IStateMachineTarget, HugEggStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		hug.pre.Face(target, 0.5f).Enter(delegate(Instance smi)
		{
		...
		}).PlayAnim((Instance smi) => GetAnims(smi).pre)
			.OnAnimQueueComplete(hug.loop);
	...
	{
		smi.sm.target.Set(smi.GetSMI<HugMonitor.Instance>().hugTarget.gameObject, smi);
	}
