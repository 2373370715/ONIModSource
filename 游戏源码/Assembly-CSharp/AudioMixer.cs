...
public class AudioMixer
{
	...
	public bool Stop(HashedString snapshot, FMOD.Studio.STOP_MODE stop_mode = FMOD.Studio.STOP_MODE.ALLOWFADEOUT)
	{
		...
		if (activeSnapshots.TryGetValue(snapshot, out var value))
		{
			...
			string[] obj = new string[5] { "Stop Snapshot: [", null, null, null, null };
			HashedString hashedString = snapshot;
			obj[1] = hashedString.ToString();
			obj[2] = "] with fadeout mode: [";
			obj[3] = stop_mode.ToString();
			obj[4] = "]";
			audioMixer.Log(string.Concat(obj));
		}
		else
		{
			AudioMixer audioMixer2 = instance;
			HashedString hashedString = snapshot;
			audioMixer2.Log("Tried to stop snapshot: [" + hashedString.ToString() + "] but it wasn't active.");
		}
		return result;
	}
	...
	{
		RuntimeManager.GetEventDescription(event_ref.Guid).getPath(out var path);
		SetSnapshotParameter(path, parameter_name, parameter_value, shouldLog);
	}
	...
	public void UpdatePersistentSnapshotParameters()
	{
		...
		string snapshotName = GetSnapshotName(AudioMixerSnapshots.Get().DuplicantCountMovingSnapshot);
		if (activeSnapshots.TryGetValue(snapshotName, out duplicantCountMovingInst))
		{
		...
		string snapshotName2 = GetSnapshotName(AudioMixerSnapshots.Get().DuplicantCountSleepingSnapshot);
		if (activeSnapshots.TryGetValue(snapshotName2, out duplicantCountSleepingInst))
		{
		...
		string snapshotName3 = GetSnapshotName(AudioMixerSnapshots.Get().DuplicantCountAttenuatorMigrated);
		if (activeSnapshots.TryGetValue(snapshotName3, out duplicantCountInst))
		{
		...
		string snapshotName4 = GetSnapshotName(AudioMixerSnapshots.Get().PulseSnapshot);
		if (activeSnapshots.TryGetValue(snapshotName4, out pulseInst))
		{
	...
	public void StartUserVolumesSnapshot()
	{
		...
		string snapshotName = GetSnapshotName(AudioMixerSnapshots.Get().UserVolumeSettingsSnapshot);
		if (!activeSnapshots.TryGetValue(snapshotName, out var value))
		{
			return;
		}
	...
	public void SetUserVolume(string bus, float value)
	{
		...
		string snapshotName = GetSnapshotName(AudioMixerSnapshots.Get().UserVolumeSettingsSnapshot);
		if (activeSnapshots.TryGetValue(snapshotName, out var value2))
		{
