...
public class TrappedDuplicantDiagnostic : ColonyDiagnostic
{
	...
	{
		icon = "overlay_power";
		AddCriterion("CheckTrapped", new DiagnosticCriterion(UI.COLONY_DIAGNOSTICS.TRAPPEDDUPLICANTDIAGNOSTIC.CRITERIA.CHECKTRAPPED, CheckTrapped));
	}
	...
	private bool CheckMinionBasicallyIdle(MinionIdentity minion)
	{
		...
		if (component.HasTag(GameTags.Idle) || component.HasTag(GameTags.RecoveringBreath) || component.HasTag(GameTags.MakingMess))
		{
			return true;
		}
		return false;
