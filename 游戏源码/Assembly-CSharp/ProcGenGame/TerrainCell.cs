...
public class TerrainCell
{
	...
	private static readonly Tag[] relaxedNoPOISpawnTagsAllowNearStart = new Tag[1] { WorldGenTags.POI };

	private static readonly TagSet relaxedNoPOISpawnAllowNearStartTagSet = new TagSet(relaxedNoPOISpawnTagsAllowNearStart);
	...
	public void InitializeCells(HashSet<int> claimedCells)
	{
		...
		for (int i = (int)poly.bounds.y; (float)i < poly.bounds.y + poly.bounds.height; i++)
		{
			for (int j = (int)poly.bounds.x; (float)j < poly.bounds.x + poly.bounds.width; j++)
			{
	...
	public void Process(WorldGen worldGen, Chunk world, SeededRandom rnd)
	{
		SetValuesFunction setValues = delegate(int index, object elem, Sim.PhysicsData pd, Sim.DiseaseCell dc)
		{
			SimMessages.ModifyCell(index, (elem as Element).idx, pd.temperature, pd.mass, dc.diseaseIdx, dc.elementCount, SimMessages.ReplaceType.Replace);
		};
		DoProcess(worldGen, world, setValues, rnd);
	}
	...
	{
		return IsSafeToSpawnPOI(allCells, relaxedNoPOISpawnTagsAllowNearStart, relaxedNoPOISpawnAllowNearStartTagSet, log);
	}
