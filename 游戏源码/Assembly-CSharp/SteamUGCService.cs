...
public class SteamUGCService : MonoBehaviour
{
	...
	private SteamUGCService()
	{
		...
		on_download_completed = Callback<DownloadItemResult_t>.Create(OnDownloadItemComplete);
		mods = new List<Mod>();
	}
	...
	private void Update()
	{
		...
		foreach (PublishedFileId_t download in downloads)
		{
			...
			if (((itemState & EItemState.k_EItemStateInstalled) == 0 || (itemState & EItemState.k_EItemStateNeedsUpdate) != 0) && (itemState & (EItemState.k_EItemStateDownloading | EItemState.k_EItemStateDownloadPending)) == 0 && SteamUGC.DownloadItem(download, bHighPriority: false))
			{
				awaiting_download.Add(download);
			}
		}
		if (details_query == UGCQueryHandle_t.Invalid)
		{
			queries.UnionWith(downloads);
			queries.ExceptWith(awaiting_download);
			downloads.Clear();
			if (queries.Count != 0)
			{
	...
	private void OnSteamUGCQueryDetailsCompleted(SteamUGCQueryCompleted_t pCallback, bool bIOFailure)
	{
		...
		case EResult.k_EResultOK:
		{
			for (uint num = 0u; num < pCallback.m_unNumResultsReturned; num++)
			{
				...
				if (SteamUGC.GetQueryUGCResult(details_query, num, out pDetails))
				{
	...
	private void OnDownloadItemComplete(DownloadItemResult_t callback)
	{
		...
		{
			queries.Add(callback.m_nPublishedFileId);
			awaiting_download.Remove(callback.m_nPublishedFileId);
		}
	}
