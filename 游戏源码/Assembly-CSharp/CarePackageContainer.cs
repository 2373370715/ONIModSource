...
public class CarePackageContainer : KScreen, ITelepadDeliverableContainer
{
	...
	private void GenerateCharacter(bool is_starter)
	{
		...
		{
			carePackageInstanceData.facadeID = Db.GetEquippableFacades().resources.FindAll((EquippableFacadeResource match) => match.DefID == info.id).GetRandom().Id;
		}
		else
		{
	...
	private string GetCurrentQuantity(WorldInventory inventory)
	{
		...
		float amount2 = inventory.GetAmount(info.id.ToTag(), includeRelatedWorlds: false);
