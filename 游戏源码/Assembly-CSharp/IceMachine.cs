...
public class IceMachine : StateMachineComponent<IceMachine.StatesInstance>, FewOptionSideScreen.IFewOptionSideScreen
{
	...
	private void MakeIce(StatesInstance smi, float dt)
	{
		...
		for (int num2 = waterStorage.items.Count; num2 > 0; num2--)
		{
			...
			if ((bool)gameObject && gameObject.GetComponent<PrimaryElement>().Temperature < gameObject.GetComponent<PrimaryElement>().Element.lowTemp)
			{
				...
				waterStorage.AddOre(targetProductionElement, component.Mass, component.Temperature, component.DiseaseIdx, component.DiseaseCount);
	...
	public FewOptionSideScreen.IFewOptionSideScreen.Option[] GetOptions()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			array[i] = new FewOptionSideScreen.IFewOptionSideScreen.Option(IceMachineConfig.ELEMENT_OPTIONS[i], ElementLoader.GetElement(IceMachineConfig.ELEMENT_OPTIONS[i]).name, Def.GetUISprite(IceMachineConfig.ELEMENT_OPTIONS[i]), tooltipText);
		}
		return array;
	}
	...
	{
		targetProductionElement = ElementLoader.GetElementID(option.tag);
	}

	public Tag GetSelectedOption()
	{
		return targetProductionElement.CreateTag();
	}
}
