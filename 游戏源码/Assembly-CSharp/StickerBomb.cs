...
public class StickerBomb : StateMachineComponent<StickerBomb.StatesInstance>
{
	...
	{
		if (stickerName.IsNullOrWhiteSpace())
		{
			Debug.LogError("Missing sticker db entry for " + stickerType);
		}
		else
		{
			DbStickerBomb dbStickerBomb = Db.GetStickerBombs().Get(stickerName);
			GetComponent<KBatchedAnimController>().SwapAnims(new KAnimFile[1] { dbStickerBomb.animFile });
		}
	...
	{
		if (stickerName.IsNullOrWhiteSpace() && !stickerType.IsNullOrWhiteSpace())
		{
			string[] array = stickerType.Split('_');
			if (array.Length == 2)
			{
				stickerName = array[1];
			}
	...
	public void SetStickerType(string newStickerType)
	{
		...
		stickerName = randomSticker.Id;
		stickerType = $"{newStickerType}_{randomSticker.Id}";
		GetComponent<KBatchedAnimController>().SwapAnims(new KAnimFile[1] { randomSticker.animFile });
	}
}
