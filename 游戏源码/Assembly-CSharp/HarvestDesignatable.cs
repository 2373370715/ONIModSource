...
public class HarvestDesignatable : KMonoBehaviour
{
	...
	protected override void OnSpawn()
	{
		...
		Game.Instance.Subscribe(1983128072, RefreshOverlayIcon);
	...
	protected override void OnCleanUp()
	{
		...
		Game.Instance.Unsubscribe(1983128072, RefreshOverlayIcon);
	}
	...
	private void CreateOverlayIcon()
	{
		if (!(HarvestWhenReadyOverlayIcon != null) && GetComponent<AttackableBase>() == null)
		{
			...
			TransformExtensions.SetPosition(position: (!GetComponent<KPrefabID>().HasTag(GameTags.Hanging)) ? (new Vector3((float)(extents.x + extents.width / 2) + 0.5f, extents.y) + (Vector3)iconOffset) : (new Vector3((float)(extents.x + extents.width / 2) + 0.5f, extents.y + extents.height) + (Vector3)iconOffset), transform: HarvestWhenReadyOverlayIcon.transform);
			RefreshOverlayIcon();
		}
	}
