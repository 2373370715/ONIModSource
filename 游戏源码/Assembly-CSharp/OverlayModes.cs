...
public abstract class OverlayModes
{
	...
	public class Crop : BasePlantMode
	{
		...
		private void AddCropUI(HarvestDesignatable harvestable)
		{
			...
			Vector3 vector = Grid.CellToPos(Grid.PosToCell(harvestable), 0.5f, -1.25f, 0f) + (Vector3)harvestable.iconOffset;
	...
	public class SolidConveyor : Mode
	{
		...
		private void FindConnectedNetworks(int cell, IUtilityNetworkMgr mgr, ICollection<UtilityNetwork> networks, List<int> visited)
		{
			...
			if (networkForCell != null)
			{
				...
				object endpoint = mgr.GetEndpoint(cell);
				if (endpoint != null && endpoint is FlowUtilityNetwork.NetworkItem { GameObject: var gameObject } && gameObject != null)
				{
	...
	public class Temperature : Mode
	{
		...
		public List<LegendEntry> temperatureLegend = new List<LegendEntry>
		{
			...
			new LegendEntry(UI.OVERLAYS.TEMPERATURE.HEATSOURCES, UI.OVERLAYS.TEMPERATURE.TOOLTIPS.HEATSOURCES, Color.white, null, Assets.GetSprite("heat_source")),
			new LegendEntry(UI.OVERLAYS.TEMPERATURE.HEATSINK, UI.OVERLAYS.TEMPERATURE.TOOLTIPS.HEATSINK, Color.white, null, Assets.GetSprite("heat_sink"))
		};
		...
		public override List<LegendEntry> GetCustomLegendData()
		{
			return Game.Instance.temperatureOverlayMode switch
			{
				...
				Game.TemperatureOverlayModes.RelativeTemperature => new List<LegendEntry>(), 
