...
public class SegmentedCreature : GameStateMachine<SegmentedCreature, SegmentedCreature.Instance, IStateMachineTarget, SegmentedCreature.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		{
			float num = (float)creatureBatchSlot * 0.01f;
			creatureBatchSlot = (creatureBatchSlot + 1) % 10;
			CreatureSegment value = segments.AddFirst(new CreatureSegment(GetComponent<KBatchedAnimController>(), base.gameObject, num, base.smi.def.headOffset, Vector3.zero)).Value;
			...
			for (int i = 0; i < base.def.numBodySegments; i++)
			{
				...
				CreatureSegment creatureSegment = new CreatureSegment(value.animController, gameObject, num + (float)(i + 1) * 0.0001f, Vector3.zero, pivot);
		...
		public void AnimEntered(HashedString name)
		{
			...
			{
				SetBodySegmentsVisibility(visible: false);
			}
			else
			{
				SetBodySegmentsVisibility(visible: true);
			}
		}

		public void SetBodySegmentsVisibility(bool visible)
		{
			for (LinkedListNode<CreatureSegment> linkedListNode = base.smi.GetFirstBodySegmentNode(); linkedListNode != null; linkedListNode = linkedListNode.Next)
			{
	...
	public class CreatureSegment
	{
		...
		public float ZOffset => Grid.GetLayerZ(head.sceneLayer) + zRelativeOffset;
		...
		{
			bool flag = false;
			if (animController != null && animController.sceneLayer != head.sceneLayer)
			{
				animController.SetSceneLayer(head.sceneLayer);
				flag = true;
			}
			value.z = ZOffset;
	...
	{
		float num = UpdateHeadPosition(smi);
		if (num != 0f)
		{
			AdjustBodySegmentsSpacing(smi, num);
			UpdateBodyPosition(smi);
		}
	}
