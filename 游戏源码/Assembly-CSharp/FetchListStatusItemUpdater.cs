...
public class FetchListStatusItemUpdater : KMonoBehaviour, IRender200ms
{
	...
	public void Render200ms(float dt)
	{
		foreach (WorldContainer worldContainer in ClusterManager.Instance.WorldContainers)
		{
			...
			int num = Math.Min(maxIteratingCount, fetchLists.Count - currentIterationIndex[id]);
			for (int i = 0; i < num; i++)
			{
				FetchList2 fetchList = fetchLists[i + currentIterationIndex[id]];
			...
			currentIterationIndex[id] += num;
			if (currentIterationIndex[id] >= fetchLists.Count)
			{
				currentIterationIndex[id] = 0;
			}
