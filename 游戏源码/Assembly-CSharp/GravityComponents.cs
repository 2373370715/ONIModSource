...
public class GravityComponents : KGameObjectComponentManager<GravityComponent>
{
	...
	public override void FixedUpdate(float dt)
	{
		...
		{
			GravityComponent gravityComponent = data[i];
			if (gravityComponent.elapsedTime < 0f || gravityComponent.transform == null || IsInCleanupList(gravityComponent.transform.gameObject))
			{
				continue;
			}
			...
			if (Grid.IsValidCell(num6))
			{
				if (vector2.sqrMagnitude > 0.2f && Grid.IsValidCell(num2) && !Grid.Element[num2].IsLiquid && Grid.Element[num6].IsLiquid)
				{
					...
					if (ambience != AmbienceType.None)
					{
						...
						{
							SoundEvent.PlayOneShot(event_ref, vector3);
						}
					}
				}
				...
				if (flag4)
				{
					vector3.y = Grid.CellToPosCBC(Grid.CellAbove(num6), Grid.SceneLayer.Move).y + groundOffset;
			...
			data[i] = gravityComponent;
			int cell2 = Grid.PosToCell(vector3);
			if (!gravityComponent.mayLeaveWorld && Grid.IsValidCell(num2) && Grid.WorldIdx[num2] != byte.MaxValue && !Grid.IsValidCellInWorld(cell2, Grid.WorldIdx[num2]))
			{
				continue;
			}
