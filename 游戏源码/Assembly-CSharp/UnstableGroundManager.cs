...
public class UnstableGroundManager : KMonoBehaviour
{
	...
	protected override void OnPrefabInit()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			GameObjectPool pool = new GameObjectPool(() => InstantiateObj(prefab), 16);
			value.pool = pool;
	...
	private void Update()
	{
		...
		while (num < fallingObjects.Count)
		{
			...
			if (!Grid.IsValidCell(num2) || Grid.Element[num2].IsSolid || (Grid.Properties[num2] & 4u) != 0)
			{
				...
				SimMessages.AddRemoveSubstance(num3, component.element, CellEventLogger.Instance.UnstableGround, component.mass, component.temperature, component.diseaseIdx, component.diseaseCount, do_vertical_solid_displacement: true, handle.index);
