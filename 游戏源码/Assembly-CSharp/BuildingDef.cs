...
public class BuildingDef : Def
{
	...
	public bool CanReplace(GameObject go)
	{
		...
		return go.GetComponent<KPrefabID>().HasAnyTags(ReplacementTags);
	}
	...
	{
		GameObject gameObject = Build(cell, orientation, resource_storage, selected_elements, temperature, playsound, timeBuilt);
	...
	{
		return TryPlace(src_go, pos, orientation, selected_elements, null);
	}
	...
	{
		return TryPlace(src_go, pos, orientation, selected_elements, facadeID, restrictToActiveWorld: true, layer);
	}
	...
	public GameObject TryPlace(GameObject src_go, Vector3 pos, Orientation orientation, IList<Tag> selected_elements, string facadeID, bool restrictToActiveWorld, int layer = 0)
	{
		...
		if (IsValidPlaceLocation(src_go, Grid.PosToCell(pos), orientation, replace_tile: false, out var _, restrictToActiveWorld))
		{
		...
		{
			gameObject.GetComponent<BuildingFacade>().ApplyBuildingFacade(Db.GetBuildingFacades().Get(facadeID));
			gameObject.GetComponent<KBatchedAnimController>().Play("place");
		}
		return gameObject;
	}
	...
	{
		GameObject gameObject = TryReplaceTile(src_go, pos, orientation, selected_elements, layer);
		if (gameObject != null)
		{
			if (facadeID != null && facadeID != "DEFAULT_FACADE")
			{
	...
	{
		return IsAreaClear(source_go, cell, orientation, layer, tile_layer, replace_tile, restrictToActiveWorld: true, out fail_reason);
	}
	...
	private bool IsAreaClear(GameObject source_go, int cell, Orientation orientation, ObjectLayer layer, ObjectLayer tile_layer, bool replace_tile, bool restrictToActiveWorld, out string fail_reason)
	{
		...
		for (int i = 0; i < PlacementOffsets.Length; i++)
		{
			...
			if (restrictToActiveWorld && Grid.WorldIdx[num] != ClusterManager.Instance.activeWorldId)
			{
				fail_reason = UI.TOOLTIPS.HELP_BUILDLOCATION_INVALID_CELL;
				return false;
			}
			...
			bool num2 = BuildLocationRule == BuildLocationRule.LogicBridge || BuildLocationRule == BuildLocationRule.Conduit || BuildLocationRule == BuildLocationRule.WireBridge;
			GameObject gameObject = null;
			if (replace_tile)
			{
			...
			if (!num2)
			{
				...
				if (gameObject2 != null)
				{
					...
					if (component != null)
					{
						flag2 = component.Def.BuildLocationRule == BuildLocationRule.LogicBridge || component.Def.BuildLocationRule == BuildLocationRule.Conduit || component.Def.BuildLocationRule == BuildLocationRule.WireBridge;
					}
				}
				if (!flag2)
				{
					if (gameObject2 != null && gameObject2 != source_go && (gameObject == null || gameObject != gameObject2) && (gameObject2.GetComponent<Wire>() == null || BuildingComplete.GetComponent<Wire>() == null))
					{
			...
			if (layer == ObjectLayer.Gantry)
			{
				...
				if (flag3 && !IsValidTileLocation(source_go, num, replace_tile, ref fail_reason))
				{
					flag = false;
					break;
				}
				...
				if (gameObject3 != null && gameObject3.GetComponent<BuildingPreview>() == null)
				{
					...
					if (flag3 || component2 == null || component2.Def.AttachmentSlotTag != GameTags.Rocket)
					{
						fail_reason = UI.TOOLTIPS.HELP_BUILDLOCATION_OCCUPIED;
						flag = false;
						break;
			...
			{
				if (!IsValidTileLocation(source_go, num, replace_tile, ref fail_reason))
				{
					flag = false;
					break;
				}
		...
		case BuildLocationRule.HighWattBridgeTile:
			flag = IsValidTileLocation(source_go, cell, replace_tile, ref fail_reason) && IsValidHighWattBridgeLocation(source_go, cell, orientation, out fail_reason);
			break;
		...
		case BuildLocationRule.NotInTiles:
		{
			...
			{
				GameObject gameObject6 = Grid.Objects[cell, (int)ObjectLayer];
				if (gameObject6 != null)
				{
					...
					else
					{
						...
						if (component4 != null && component4.Def.ReplacementLayer != ReplacementLayer)
						{
							flag = false;
						}
					}
		...
		if (BuildLocationRule == BuildLocationRule.LogicBridge)
		{
			...
			if (component != null && component.inputPortInfo != null)
			{
				...
				for (int j = 0; j < inputPortInfo.Length; j++)
				{
					...
					MarkOverlappingLogicPorts(Grid.Objects[cell7, (int)layer], go, cell7);
	...
	public void MarkOverlappingLogicPorts(GameObject existing, GameObject replaced, int cell)
	{
		...
		else if (existing != replaced)
		{
			...
			if ((component2 != null && component2.TryGetPortAtCell(cell, out var _, out var _)) || (component != null && component.TryGetPortAtCell(cell, out var _)))
			{
				existing.AddTag(GameTags.HasInvalidPorts);
			}
		}
	...
	{
		return IsValidPlaceLocation(source_go, cell, orientation, replace_tile, out fail_reason, restrictToActiveWorld: false);
	}
	...
	public bool IsValidPlaceLocation(GameObject source_go, int cell, Orientation orientation, bool replace_tile, out string fail_reason, bool restrictToActiveWorld)
	{
		...
		if (restrictToActiveWorld && Grid.WorldIdx[cell] != ClusterManager.Instance.activeWorldId)
		{
			fail_reason = UI.TOOLTIPS.HELP_BUILDLOCATION_INVALID_CELL;
			return false;
		}
		...
		return IsAreaClear(source_go, cell, orientation, ObjectLayer, TileLayer, replace_tile, restrictToActiveWorld, out fail_reason);
	}
	...
	{
		string reason = "";
		return IsValidBuildLocation(source_go, pos, orientation, out reason, replace_tile);
	}
	...
	{
		int cell = Grid.PosToCell(pos);
		return IsValidBuildLocation(source_go, cell, orientation, replace_tile, out reason);
	}
	...
	public void PostProcess()
	{
		...
		for (int i = 0; i < MaterialCategory.Length; i++)
		{
			TagManager.Create(MaterialCategory[i], MATERIALS.GetMaterialString(MaterialCategory[i]));
		...
		if (!Deprecated)
		{
			Db.Get().TechItems.AddTechItem(PrefabID, Name, Effect, GetUISprite, RequiredDlcIds, POIUnlockable);
		}
	}
	...
	{
		if (!CheckRequiresPowerInput() && !CheckRequiresPowerOutput() && !CheckRequiresGasInput() && !CheckRequiresGasOutput() && !CheckRequiresLiquidInput() && !CheckRequiresLiquidOutput() && !CheckRequiresSolidInput() && !CheckRequiresSolidOutput() && !CheckRequiresHighEnergyParticleInput() && !CheckRequiresHighEnergyParticleOutput() && SelfHeatKilowattsWhenActive == 0f && ExhaustKilowattsWhenActive == 0f)
		{
	...
	{
		return SaveLoader.Instance.IsDlcListActiveForCurrentSave(RequiredDlcIds);
	}
}
