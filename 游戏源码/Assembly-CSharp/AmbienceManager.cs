...
public class AmbienceManager : KMonoBehaviour
{
	...
	public class Layer : IComparable<Layer>
	{
		...
		public void Start(Vector3 emitter_position)
		{
			...
			if (!oneShotSound.IsNull)
			{
				...
				{
					EventReference eventReference = oneShotSound;
					Debug.LogWarning("Could not find event: " + eventReference.ToString());
					return;
				}
	...
	public class Quadrant
	{
		...
		public Layer rocketInteriorLayer;

		public Layer facilityLayer;

		public Layer radiationLayer;

		public Layer[] solidLayers = new Layer[19];
		...
		{
			name = def.name;
			fogLayer = new Layer(def.fogSound);
			allLayers.Add(fogLayer);
			loopingLayers.Add(fogLayer);
			spaceLayer = new Layer(def.spaceSound);
			allLayers.Add(spaceLayer);
			loopingLayers.Add(spaceLayer);
			m_isClusterSpaceEnabled = DlcManager.FeatureClusterSpaceEnabled();
			if (m_isClusterSpaceEnabled)
			{
				rocketInteriorLayer = new Layer(def.rocketInteriorSound);
				allLayers.Add(rocketInteriorLayer);
			}
			facilityLayer = new Layer(def.facilitySound);
			...
			{
				radiationLayer = new Layer(def.radiationSound);
				allLayers.Add(radiationLayer);
			}
			for (int i = 0; i < 4; i++)
			{
				gasLayers[i] = new Layer(def.gasSounds[i]);
				liquidLayers[i] = new Layer(def.liquidSounds[i]);
			...
			for (int j = 0; j < solidLayers.Length; j++)
			{
				...
				solidLayers[j] = new Layer(default(EventReference), def.solidSounds[j]);
		...
		public void Update(Vector2I min, Vector2I max, Vector3 emitter_position)
		{
			...
			if (m_isClusterSpaceEnabled)
			{
				...
				rocketInteriorLayer.Start(emitter_position);
				rocketInteriorLayer.SetCustomParameter("RocketState", (float)ClusterManager.RocketInteriorState);
				rocketInteriorLayer.SetVolume(volume);
			}
			if (m_isRadiationEnabled)
			{
