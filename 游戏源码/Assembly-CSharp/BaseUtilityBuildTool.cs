	...
	private IEnumerator VisUpdater()
	{
		while (true)
		{
			...
			for (int i = 0; i < path.Count; i++)
			{
				...
				component.TintColour = (def.IsValidBuildLocation(null, node2.cell, Orientation.Neutral, replace_tile: false, out var _) ? Color.white : Color.red);
		...
		for (int i = 0; i < path.Count; i++)
		{
			...
			if (gameObject == null)
			{
				...
				if ((DebugHandler.InstantBuildMode || (Game.Instance.SandboxModeActive && SandboxToolParameterMenu.instance.settings.InstantBuild)) && def.IsValidBuildLocation(visualizer, vector, Orientation.Neutral) && def.IsValidPlaceLocation(visualizer, vector, Orientation.Neutral, out var _))
				{
					...
					gameObject = def.Build(pathNode.cell, Orientation.Neutral, null, selectedElements, Mathf.Min(def.Temperature, b), timeBuilt: GameClock.Instance.GetTime(), facadeID: facadeID);
				}
				else
				{
					gameObject = def.TryPlace(null, vector, Orientation.Neutral, selectedElements, facadeID);
					if (gameObject != null)
					{
