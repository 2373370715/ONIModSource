...
public class GeneticAnalysisStationWorkable : Workable
{
	...
	protected override void OnStartWork(Worker worker)
	{
		...
		GetComponent<KSelectable>().AddStatusItem(Db.Get().BuildingStatusItems.ComplexFabricatorResearching, storage.FindFirst(GameTags.UnidentifiedSeed));
	}

	protected override void OnStopWork(Worker worker)
	{
		base.OnStopWork(worker);
		GetComponent<KSelectable>().RemoveStatusItem(Db.Get().BuildingStatusItems.ComplexFabricatorResearching);
	}
