...
public class TinkerStation : Workable, IGameObjectEffectDescriptor, ISim1000ms
{
	...
	protected override void OnPrefabInit()
	{
		...
		if (useFilteredStorage)
		{
			...
			filteredStorage = new FilteredStorage(this, null, null, use_logic_meter: false, byHash);
		}
		SetWorkTime(15f);
	...
	public override List<Descriptor> GetDescriptors(GameObject go)
	{
		...
		if (list.Count > 0)
		{
			...
			{
				Descriptor item = new Descriptor(string.Format(EffectItemString, item3.GetProperName()), string.Format(EffectItemTooltip, item3.GetProperName()));
				item.IncreaseIndent();
