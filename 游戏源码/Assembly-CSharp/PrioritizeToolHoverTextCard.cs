...
public class PrioritizeToolHoverTextCard : HoverTextConfiguration
{
	...
	protected override void ConfigureTitle(HoverTextScreen screen)
	{
		...
		if (lastEnabledFilter != null && lastEnabledFilter != "ALL")
		{
			ToolName = string.Format(UI.TOOLS.CAPITALS, string.Concat(Strings.Get(ToolNameStringKey), string.Format(UI.TOOLS.FILTER_HOVERCARD_HEADER, Strings.Get("STRINGS.UI.TOOLS.FILTERLAYERS." + lastEnabledFilter + ".TOOLTIP"))));
		}
	}
}
