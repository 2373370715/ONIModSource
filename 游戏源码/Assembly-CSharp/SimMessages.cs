...
public static class SimMessages
{
	...
	{
		RegisterBuildingToBuildingHeatExchangeMessage* ptr = stackalloc RegisterBuildingToBuildingHeatExchangeMessage[1];
		ptr->structureTemperatureHandler = structureTemperatureHandler;
		ptr->callbackIdx = callbackIdx;
		Sim.SIM_HandleMessage(-1338718217, sizeof(RegisterBuildingToBuildingHeatExchangeMessage), (byte*)ptr);
	}

	public unsafe static void AddBuildingToBuildingHeatExchange(int selfHandler, int buildingInContact, int cellsInContact)
	{
		AddBuildingToBuildingHeatExchangeMessage* ptr = stackalloc AddBuildingToBuildingHeatExchangeMessage[1];
		ptr->selfHandler = selfHandler;
		ptr->buildingInContactHandle = buildingInContact;
		ptr->cellsInContact = cellsInContact;
		Sim.SIM_HandleMessage(-1586724321, sizeof(AddBuildingToBuildingHeatExchangeMessage), (byte*)ptr);
	}

	public unsafe static void RemoveBuildingInContactFromBuildingToBuildingHeatExchange(int selfHandler, int buildingToRemove)
	{
		RemoveBuildingInContactFromBuildingToBuildingHeatExchangeMessage* ptr = stackalloc RemoveBuildingInContactFromBuildingToBuildingHeatExchangeMessage[1];
		ptr->selfHandler = selfHandler;
		ptr->buildingNoLongerInContactHandler = buildingToRemove;
		Sim.SIM_HandleMessage(-1993857213, sizeof(RemoveBuildingInContactFromBuildingToBuildingHeatExchangeMessage), (byte*)ptr);
	}

	public unsafe static void RemoveBuildingToBuildingHeatExchange(int selfHandler, int callback = -1)
	{
		RemoveBuildingToBuildingHeatExchangeMessage* ptr = stackalloc RemoveBuildingToBuildingHeatExchangeMessage[1];
		ptr->callbackIdx = callback;
		ptr->selfHandler = selfHandler;
		Sim.SIM_HandleMessage(697100730, sizeof(RemoveBuildingToBuildingHeatExchangeMessage), (byte*)ptr);
	}
	...
	public unsafe static void ModifyCell(int gameCell, ushort elementIdx, float temperature, float mass, byte disease_idx, int disease_count, ReplaceType replace_type = ReplaceType.None, bool do_vertical_solid_displacement = false, int callbackIdx = -1)
	{
		if (Grid.IsValidCell(gameCell))
		{
			...
			ptr->diseaseIdx = disease_idx;
			ptr->diseaseCount = disease_count;
			ptr->addSubType = ((!do_vertical_solid_displacement) ? ((byte)1) : ((byte)0));
	...
	{
		ushort elementIndex = GetElementIndex(new_element);
	...
	public static void ModifyMass(int gameCell, float mass, byte disease_idx, int disease_count, CellModifyMassEvent ev, float temperature = -1f, SimHashes element = SimHashes.Vacuum)
	{
		...
		{
			ushort elementIndex = GetElementIndex(element);
			if (elementIndex != ushort.MaxValue)
			{
				if (temperature == -1f)
				{
	...
	public unsafe static void NewGameFrame(float elapsed_seconds, List<Game.SimActiveRegion> activeRegions)
	{
		...
		foreach (Game.SimActiveRegion activeRegion in activeRegions)
		{
			...
			region.second = new Vector2I(MathUtil.Clamp(0, Grid.WidthInCells, activeRegion.region.second.x), MathUtil.Clamp(0, Grid.HeightInCells - 1, activeRegion.region.second.y));
