...
public class CarePackage : StateMachineComponent<CarePackage.SMInstance>
{
	...
	{
		return new EmoteReactable(base.gameObject, "UpgradeFX", Db.Get().ChoreTypes.Emote).SetEmote(Db.Get().Emotes.Minion.Cheer);
	}
	...
	private void SetAnimToInfo()
	{
		...
		{
			component4.SwapAnims(new KAnimFile[1] { Db.GetEquippableFacades().Get(facadeID).AnimFile });
