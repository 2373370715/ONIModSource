...
public class ReportScreenEntryRow : KMonoBehaviour
{
	...
	public void SetLine(ReportManager.ReportEntry entry, ReportManager.ReportGroup reportGroup)
	{
		...
		else
		{
			...
			float minWidth = (component.preferredWidth = nameWidth - indentWidth);
			component.minWidth = minWidth;
		...
		if (addedValue != entry.Positive)
		{
			...
			{
				float num3 = 0f;
				num3 = ((entry.contextEntries.Count <= 0) ? ((float)pos_notes.Count) : ((float)entry.contextEntries.Count));
				num3 = Mathf.Max(num3, 1f);
				text = reportGroup.groupFormatfn(entry.Positive, num3);
			}
			added.text = text;
		...
		if (removedValue != entry.Negative)
		{
			...
			{
				float num4 = 0f;
				num4 = ((entry.contextEntries.Count <= 0) ? ((float)neg_notes.Count) : ((float)entry.contextEntries.Count));
				num4 = Mathf.Max(num4, 1f);
				text2 = reportGroup.groupFormatfn(0f - entry.Negative, num4);
			}
			removed.text = text2;
		...
		if (netValue != entry.Net)
		{
			...
			{
				float num5 = 0f;
				num5 = ((entry.contextEntries.Count <= 0) ? ((float)(pos_notes.Count + neg_notes.Count)) : ((float)entry.contextEntries.Count));
				num5 = Mathf.Max(num5, 1f);
				text3 = reportGroup.groupFormatfn(entry.Net, num5);
			}
			net.text = text3;
			netValue = entry.Net;
		}
