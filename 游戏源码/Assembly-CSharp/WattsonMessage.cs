...
public class WattsonMessage : KScreen
{
	...
	protected override void OnPrefabInit()
	{
		...
		{
			message.SetText(Strings.TryGet(welcomeMessage, out var result) ? result.String : welcomeMessage);
		}
	...
	private IEnumerator ExpandPanel()
	{
		...
		{
			dialog.rectTransform().rotation = Quaternion.Euler(0f, 0f, -90f);
		}
		...
		if (CustomGameSettings.Instance.GetSettingsCoordinate().StartsWith("KF23"))
		{
			...
			while (t < duration)
			{
				...
				dialog.rectTransform().rotation = Quaternion.Slerp(initialOrientation, Quaternion.identity, t / duration);
				yield return 0;
			}
		}
		yield return null;
	...
	protected override void OnActivate()
	{
		...
		if (telepad != null)
		{
			...
			for (int i = 0; i < Components.LiveMinionIdentities.Count; i++)
			{
				...
				UIScheduler.Instance.Schedule("DupeBirth", (float)idx * 0.5f, delegate
				{
					...
					EmoteChore emoteChore = new EmoteChore(chore_provider, Db.Get().ChoreTypes.EmoteHighPriority, "anim_interacts_portal_kanim", new HashedString[1] { "portalbirth_" + idx });
		...
		{
			CameraController.Instance.OrthographicSize = TuningData<Tuning>.Get().initialOrthographicSize;
			CameraController.Instance.CameraGoHome(0.5f);
			startFade = true;
			MusicManager.instance.PlaySong(WelcomeMusic);
		}));
	}
	...
	protected override void OnDeactivate()
	{
		...
		MusicManager.instance.StopSong(WelcomeMusic);
