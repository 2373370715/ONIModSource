...
public class DeconstructToolHoverTextCard : HoverTextConfiguration
{
	...
	protected override void ConfigureTitle(HoverTextScreen screen)
	{
		...
		{
			ToolName = string.Format(UI.TOOLS.CAPITALS, Strings.Get(ToolNameStringKey).String + string.Format(UI.TOOLS.FILTER_HOVERCARD_HEADER, Strings.Get("STRINGS.UI.TOOLS.FILTERLAYERS." + lastEnabledFilter + ".TOOLTIP").String));
		}
