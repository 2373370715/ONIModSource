...
public class BalloonArtist : GameStateMachine<BalloonArtist, BalloonArtist.Instance>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void Internal_InitBalloons()
		{
			...
			if (balloonSymbolIter.IsNullOrDestroyed() || !(balloonSymbolIter.facade.AndThen((BalloonArtistFacadeResource f) => f.Id) == joyResponseOutfitTarget.ReadFacadeId()))
			{
				balloonSymbolIter = joyResponseOutfitTarget.ReadFacadeId().AndThen((string id) => Db.Get().Permits.BalloonArtistFacades.Get(id)).AndThen((BalloonArtistFacadeResource permit) => permit.GetSymbolIter())
					.UnwrapOr(new BalloonOverrideSymbolIter(Option.None));
				SetBalloonSymbolOverride(balloonSymbolIter.Current());
			}
		}
		...
		public void SetBalloonSymbolOverride(BalloonOverrideSymbol balloonOverrideSymbol)
		{
			...
			{
				base.master.GetComponent<SymbolOverrideController>().AddSymbolOverride("body", Assets.GetAnim("balloon_anim_kanim").GetData().build.GetSymbol("body"));
			}
			else
			{
				base.master.GetComponent<SymbolOverrideController>().AddSymbolOverride("body", balloonOverrideSymbol.symbol.Unwrap());
			}
		}

		public BalloonOverrideSymbol GetCurrentBalloonSymbolOverride()
		{
			return balloonSymbolIter.Current();
		}

		public void ApplyNextBalloonSymbolOverride()
		{
			SetBalloonSymbolOverride(balloonSymbolIter.Next());
		}
