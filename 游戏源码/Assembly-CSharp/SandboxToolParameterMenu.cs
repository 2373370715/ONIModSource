...
public class SandboxToolParameterMenu : KScreen
{
	...
	public SelectorValue storySelector;
	...
	private void ConfigureSettings()
	{
		...
		sandboxSettings5.OnChangeStory = (System.Action)Delegate.Combine(sandboxSettings5.OnChangeStory, (System.Action)delegate
		{
			string stringSetting2 = instance.settings.GetStringSetting("SandboxTools.SelectedStory");
			Story story = Db.Get().Stories.Get(stringSetting2);
			if (story == null)
			{
				settings.ForceDefaultStringSetting("SandboxTools.SelectedStory");
			}
			else
			{
				storySelector.button.GetComponentInChildren<LocText>().text = Strings.Get(story.StoryTrait.name);
				storySelector.button.GetComponentsInChildren<Image>()[1].sprite = Assets.GetSprite(story.StoryTrait.icon);
			}
		});
		SandboxSettings sandboxSettings6 = settings;
		sandboxSettings6.OnChangeEntity = (System.Action)Delegate.Combine(sandboxSettings6.OnChangeEntity, (System.Action)delegate
		{
		...
		sandboxSettings7.OnChangeBrushSize = (System.Action)Delegate.Combine(sandboxSettings7.OnChangeBrushSize, (System.Action)delegate
		{
		...
		sandboxSettings8.OnChangeNoiseScale = (System.Action)Delegate.Combine(sandboxSettings8.OnChangeNoiseScale, (System.Action)delegate
		{
		...
		sandboxSettings9.OnChangeNoiseDensity = (System.Action)Delegate.Combine(sandboxSettings9.OnChangeNoiseDensity, (System.Action)delegate
		{
			if (PlayerController.Instance.ActiveTool is SandboxSprinkleTool)
			{
				(PlayerController.Instance.ActiveTool as SandboxSprinkleTool).SetBrushSize(settings.GetIntSetting("SandboxTools.BrushSize"));
		...
		sandboxSettings10.OnChangeTemperature = (System.Action)Delegate.Combine(sandboxSettings10.OnChangeTemperature, (System.Action)delegate
		{
		...
		sandboxSettings11.OnChangeAdditiveTemperature = (System.Action)Delegate.Combine(sandboxSettings11.OnChangeAdditiveTemperature, (System.Action)delegate
		{
		...
		sandboxSettings12.OnChangeAdditiveStress = (System.Action)Delegate.Combine(sandboxSettings12.OnChangeAdditiveStress, (System.Action)delegate
		{
			stressAdditiveSlider.SetValue(settings.GetFloatSetting("SandbosTools.StressAdditive"), runOnValueChanged: false);
		});
		SandboxSettings sandboxSettings13 = settings;
		sandboxSettings13.OnChangeMoraleAdjustment = (System.Action)Delegate.Combine(sandboxSettings13.OnChangeMoraleAdjustment, (System.Action)delegate
		{
	...
	public void DisableParameters()
	{
		...
		storySelector.row.SetActive(value: false);
	}
	...
	private void ConfigureEntitySelector()
	{
		...
		SelectorValue.SearchFilter item = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.FOOD, delegate(object entity)
		{
			...
			string idString = kPrefabID4.PrefabID().ToString();
			return (!kPrefabID4.HasTag(GameTags.Egg) && EdiblesManager.GetAllFoodTypes().Find((EdiblesManager.FoodInfo match) => match.Id == idString) != null) || kPrefabID4.HasTag(GameTags.Dehydrated);
		}, null, Def.GetUISprite(Assets.GetPrefab("MushBar")));
		list.Add(item);
		SelectorValue.SearchFilter item2 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.COMETS, delegate(object entity)
		{
		...
		}, null, Def.GetUISprite(Assets.GetPrefab(CopperCometConfig.ID)));
		list.Add(item2);
		SelectorValue.SearchFilter item3 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.SPECIAL, delegate(object entity)
		{
		...
		}, null, new Tuple<Sprite, Color>(Assets.GetSprite("ui_duplicant_portrait_placeholder"), Color.white));
		list.Add(item3);
		...
		foreach (GameObject item8 in Assets.GetPrefabsWithTag(GameTags.CreatureBrain))
		{
		...
		SelectorValue.SearchFilter item5 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.EQUIPMENT, delegate(object entity)
		{
			...
			return gameObject3 != null && gameObject3.GetComponent<Equippable>() != null;
		...
		SelectorValue.SearchFilter searchFilter3 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.PLANTS, delegate(object entity)
		{
			...
			if (kPrefabID == null || kPrefabID.gameObject == null)
			{
				return false;
			}
			return kPrefabID != null && SaveLoader.Instance.IsDlcListActiveForCurrentSave(kPrefabID.requiredDlcIds) && (kPrefabID.GetComponent<Harvestable>() != null || kPrefabID.GetComponent<WiltCondition>() != null);
		...
		{
			if (!SaveLoader.Instance.IsDlcListActiveForCurrentSave(prefab2.requiredDlcIds) || prefab2.HasTag(GameTags.HideFromSpawnTool))
			{
				continue;
			}
	...
	private void ConfigureStoryTraitSelector()
	{
		...
		storySelector = new SelectorValue(options, delegate(object story)
		{
			settings.SetStringSetting("SandboxTools.SelectedStory", ((Story)story).Id);
		}, (object story) => Strings.Get((story as Story).StoryTrait.name), null, (object story) => new Tuple<Sprite, Color>(Assets.GetSprite(((Story)story).StoryTrait.icon), Color.white), UI.SANDBOXTOOLS.SETTINGS.SPAWN_STORY_TRAIT.NAME);
	}
