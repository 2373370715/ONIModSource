...
public class Navigator : StateMachineComponent<Navigator.StatesInstance>, ISaveLoadableDetails
{
	...
	private void SubscribeUnstuckFunctions()
	{
		...
		{
			GameScenePartitioner.Instance.AddGlobalLayerListener(GameScenePartitioner.Instance.objectLayers[1], OnBuildingTileChanged);
		}
	}

	private void UnsubscribeUnstuckFunctions()
	{
		GameScenePartitioner.Instance.RemoveGlobalLayerListener(GameScenePartitioner.Instance.objectLayers[1], OnBuildingTileChanged);
	}
	...
	public void AdvancePath(bool trigger_advance = true)
	{
		...
		{
			bool atNextNode;
			bool num2 = !ValidatePath(ref path, out atNextNode);
			if (atNextNode)
			{
