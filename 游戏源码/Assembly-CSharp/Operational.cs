...
public class Operational : KMonoBehaviour
{
	...
	{
		return stateRequirement switch
		{
			State.Operational => IsOperational, 
			State.Functional => IsFunctional, 
	...
	public static GameHashes GetEventForState(State state)
	{
		return state switch
		{
			State.Operational => GameHashes.OperationalChanged, 
			State.Functional => GameHashes.FunctionalChanged, 
			_ => GameHashes.ActiveChanged, 
		};
	}
}
