...
public class Storage : Workable, ISaveLoadableDetails, IGameObjectEffectDescriptor, IStorage
{
	...
	public PrimaryElement AddElement(SimHashes element, float mass, float temperature, byte disease_idx, int disease_count, bool keep_zero_mass = false, bool do_disease_transfer = true)
	{
		...
		if (element2.IsGas)
		{
			return AddGasChunk(element, mass, temperature, disease_idx, disease_count, keep_zero_mass, do_disease_transfer);
		}
		if (element2.IsLiquid)
		{
			return AddLiquid(element, mass, temperature, disease_idx, disease_count, keep_zero_mass, do_disease_transfer);
		}
		if (element2.IsSolid)
		{
			return AddOre(element, mass, temperature, disease_idx, disease_count, keep_zero_mass, do_disease_transfer);
		}
		return null;
	}
	...
	{
		float num = amount;
		while (num > 0f && GetAmountAvailable(tag) > 0f)
		{
			num -= Transfer(dest_storage, tag, num, block_events, hide_popups);
		}
		if (flatten)
		{
	...
	public GameObject[] DropUnlessHasTag(Tag tag)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			if (!(items[i] == null) && !items[i].GetComponent<KPrefabID>().HasTag(tag))
			{
	...
	public GameObject[] DropHasTags(Tag[] tag)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			if (!(items[i] == null) && items[i].GetComponent<KPrefabID>().HasAllTags(tag))
			{
				GameObject gameObject = items[i];
				items.RemoveAt(i);
				i--;
	...
	private void MakeWorldActive(GameObject go)
	{
		...
		if (dropOffset != Vector2.zero)
		{
			go.transform.Translate(dropOffset);
		}
	...
	{
		GameObject gameObject = FindFirst(tag_to_combine);
		PrimaryElement component = gameObject.GetComponent<PrimaryElement>();
		for (int num = items.Count - 1; num >= 0; num--)
		{
			GameObject gameObject2 = items[num];
			if (gameObject2.HasTag(tag_to_combine) && gameObject2 != gameObject)
			{
				PrimaryElement component2 = gameObject2.GetComponent<PrimaryElement>();
				component.Mass += component2.Mass;
				ConsumeIgnoringDisease(gameObject2);
			}
		}
	}
	...
	{
		ConsumeAllIgnoringDisease(Tag.Invalid);
	}
	...
	public void ConsumeAllIgnoringDisease(Tag tag)
	{
		for (int num = items.Count - 1; num >= 0; num--)
		{
			if (!(tag != Tag.Invalid) || items[num].HasTag(tag))
			{
	...
	{
		return (float)Mathf.RoundToInt(ExactMassStored() * 1000f) / 1000f;
	}
	...
	private void OnQueueDestroyObject(object data)
	{
		...
		DropAll(vent_gas: true, dump_liquid: false, default(Vector3), do_disease_transfer: true, collect_dropped_items);
		if (onDestroyItemsDropped != null)
		{
			onDestroyItemsDropped(collect_dropped_items);
		}
		OnCleanUp();
	}
	...
	public void SetContentsDeleteOffGrid(bool delete_off_grid)
	{
		for (int i = 0; i < items.Count; i++)
		{
			Pickupable component = items[i].GetComponent<Pickupable>();
			if (component != null)
			{
				component.deleteOffGrid = delete_off_grid;
			}
			Storage component2 = items[i].GetComponent<Storage>();
			if (component2 != null)
			{
