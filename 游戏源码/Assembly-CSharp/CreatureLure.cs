...
public class CreatureLure : StateMachineComponent<CreatureLure.StatesInstance>
{
	...
	protected void CreateFetchChore()
	{
		...
		if (!(activeBaitSetting == Tag.Invalid))
		{
			fetchChore = new FetchChore(Db.Get().ChoreTypes.RanchingFetch, baitStorage, 100f, new HashSet<Tag> { activeBaitSetting }, FetchChore.MatchCriteria.MatchID, Tag.Invalid, null, null, run_until_complete: true, null, null, null, Operational.State.None);
