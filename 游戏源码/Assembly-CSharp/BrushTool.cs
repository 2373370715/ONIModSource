...
public class BrushTool : InterfaceTool
{
	...
	public override void OnLeftClickDown(Vector3 cursor_pos)
	{
		...
		{
			SetCurrentVirtualInputModuleMousMovementMode(mouseMovementOnly: true);
		}
		Paint();
	}
	...
	{
		string dragSound = GetDragSound();
		if (!string.IsNullOrEmpty(dragSound))
		{
			string sound = GlobalAssets.GetSound(dragSound);
			if (sound != null)
			{
				Vector3 pos = Grid.CellToPos(currentCell);
				pos.z = 0f;
				int cellDistance = Grid.GetCellDistance(Grid.PosToCell(downPos), currentCell);
