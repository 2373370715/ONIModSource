...
public class ArtifactAnalysisStationWorkable : Workable
{
	...
	private void InitialDisplayStoredArtifact()
	{
		...
		PositionArtifact();
		GetComponent<KSelectable>().AddStatusItem(Db.Get().BuildingStatusItems.ArtifactAnalysisAnalyzing, gameObject);
	}
	...
	private void ReleaseStoredArtifact()
	{
		...
		GetComponent<KSelectable>().RemoveStatusItem(Db.Get().BuildingStatusItems.ArtifactAnalysisAnalyzing, gameObject);
	}
	...
	private void YieldPayload(SpaceArtifact artifact)
	{
		...
		for (int i = 0; i < num; i++)
		{
			GameUtil.KInstantiate(Assets.GetPrefab("OrbitalResearchDatabank"), statesInstance.master.transform.position + finishedArtifactDropOffset, Grid.SceneLayer.Ore).SetActive(value: true);
		}
		nextYeildRoll = Random.Range(0f, 1f);
	}
}
