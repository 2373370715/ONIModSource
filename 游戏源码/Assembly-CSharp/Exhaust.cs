...
public class Exhaust : KMonoBehaviour, ISim200ms
{
	...
	{
		FallingWater.instance.AddParticle(cell, primary_element.Element.idx, primary_element.Mass, primary_element.Temperature, primary_element.DiseaseIdx, primary_element.DiseaseCount, skip_sound: true, skip_decor: false, debug_track: true);
	};
	...
	protected override void OnPrefabInit()
	{
		...
		GetComponent<RequireInputs>().visualizeRequirements = RequireInputs.Requirements.NoWire;
	...
	private bool EmitCommon(int cell, PrimaryElement primary_element, EmitDelegate emit)
	{
		...
		if (lastElementEmmited != primary_element.ElementID)
		{
			lastElementEmmited = primary_element.ElementID;
			if (primary_element.Element != null && primary_element.Element.substance != null)
			{
				Trigger(-793429877, primary_element.Element.substance.colour);
			}
		}
