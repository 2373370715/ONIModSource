...
public class SleepChore : Chore<SleepChore.StatesInstance>
{
	...
	public class States : GameStateMachine<States, StatesInstance, SleepChore>
	{
		public class SleepStates : State
		{
			...
			public State interrupt_cold;

			public State interrupt_cold_transition;
		}
		...
		public BoolParameter isDisturbedByCold;
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			sleep.Enter("SetAnims", delegate(StatesInstance smi)
			{
				...
				.Toggle("Custom Status Items", DisplayCustomStatusItemsWhenAsleep, RemoveCustomStatusItemsWhenAsleep)
				...
				.EventHandler(GameHashes.SleepDisturbedByCold, delegate(StatesInstance smi)
				{
					isDisturbedByCold.Set(value: true, smi);
				});
			...
			sleep.normal.ParamTransition(isInterruptable, sleep.uninterruptable, GameStateMachine<States, StatesInstance, SleepChore, object>.IsFalse).ToggleCategoryStatusItem(Db.Get().StatusItemCategories.Main, Db.Get().DuplicantStatusItems.Sleeping).QueueAnim("working_loop", loop: true)
				...
				.ParamTransition(isDisturbedByCold, sleep.interrupt_cold, GameStateMachine<States, StatesInstance, SleepChore, object>.IsTrue)
			...
			sleep.interrupt_scared_transition.Enter(delegate(StatesInstance smi)
			{
				...
				State state5 = (smi.master.GetComponent<Schedulable>().IsAllowed(Db.Get().ScheduleBlockTypes.Sleep) ? sleep.normal : success);
				isScaredOfDark.Set(value: false, smi);
				smi.GoTo(state5);
			});
			sleep.interrupt_movement.ToggleCategoryStatusItem(Db.Get().StatusItemCategories.Main, Db.Get().DuplicantStatusItems.SleepingInterruptedByMovement).PlayAnim("interrupt_light").OnAnimQueueComplete(sleep.interrupt_movement_transition)
				.Enter(delegate(StatesInstance smi)
				{
					GameObject gameObject2 = smi.sm.bed.Get(smi);
					if (gameObject2 != null)
					{
						gameObject2.Trigger(-717201811);
					}
				});
			...
			sleep.interrupt_cold.ToggleCategoryStatusItem(Db.Get().StatusItemCategories.Main, Db.Get().DuplicantStatusItems.SleepingInterruptedByCold).PlayAnim("interrupt_cold").ToggleThought(Db.Get().Thoughts.Cold)
				.OnAnimQueueComplete(sleep.interrupt_cold_transition)
				.Enter(delegate(StatesInstance smi)
				{
					GameObject gameObject = smi.sm.bed.Get(smi);
					if (gameObject != null)
					{
						gameObject.Trigger(157165762);
					}
				});
			sleep.interrupt_cold_transition.Enter(delegate(StatesInstance smi)
			{
				if (!smi.master.GetComponent<Effects>().HasEffect(Db.Get().effects.Get("TerribleSleep")))
				{
					smi.master.GetComponent<Effects>().Add(Db.Get().effects.Get("BadSleepCold"), should_save: true);
				}
				State state3 = (smi.master.GetComponent<Schedulable>().IsAllowed(Db.Get().ScheduleBlockTypes.Sleep) ? sleep.normal : success);
				isDisturbedByCold.Set(value: false, smi);
				smi.GoTo(state3);
			});
	...
	public static void DisplayCustomStatusItemsWhenAsleep(StatesInstance smi)
	{
	...
	public static void RemoveCustomStatusItemsWhenAsleep(StatesInstance smi)
	{
		if (smi.optional_StatusItemsDisplayedWhileAsleep != null)
		{
			KSelectable component = smi.gameObject.GetComponent<KSelectable>();
