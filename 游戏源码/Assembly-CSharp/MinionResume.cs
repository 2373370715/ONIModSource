...
public class MinionResume : KMonoBehaviour, ISaveLoadable, ISim200ms
{
	...
	{
		foreach (MinionResume item in ((worldId >= 0) ? Components.MinionResumes.GetWorldItems(worldId, checkChildWorlds: true) : Components.MinionResumes.Items).Where((MinionResume minion) => !minion.HasTag(GameTags.Dead)).ToList())
		{
