...
public class RailGunPayload : GameStateMachine<RailGunPayload, RailGunPayload.StatesInstance, IStateMachineTarget, RailGunPayload.Def>
{
	...
	public class StatesInstance : GameInstance
	{
		...
		public void StartLand()
		{
			...
			{
				num2 = ClusterManager.Instance.GetLandingBeaconLocation(worldContainer.id);
			}
			if (num2 != Grid.InvalidCell)
			{
				Grid.CellToXY(num2, out var x, out var _);
				int minInclusive = Mathf.Max(x - 3, (int)worldContainer.minimumBounds.x);
				int maxExclusive = Mathf.Min(x + 3, (int)worldContainer.maximumBounds.x);
				num = Mathf.RoundToInt(Random.Range(minInclusive, maxExclusive));
			}
			else
			{
				num = Mathf.RoundToInt(Random.Range(worldContainer.minimumBounds.x + 3f, worldContainer.maximumBounds.x - 3f));
			}
			TransformExtensions.SetPosition(position: new Vector3((float)num + 0.5f, worldContainer.maximumBounds.y - 1f, Grid.GetLayerZ(Grid.SceneLayer.Front)), transform: base.transform);
			if (GameComps.Fallers.Has(base.gameObject))
			{
				GameComps.Fallers.Remove(base.gameObject);
			}
		...
		public void MoveToWorld()
		{
			...
			if (component2 != null)
			{
				component2.SetContentsDeleteOffGrid(delete_off_grid: true);
			}
		}
	}
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		travel.transferWorlds.Exit(delegate(StatesInstance smi)
		{
			smi.StartLand();
