	...
	public class States : GameStateMachine<States, StatesInstance, EatChore>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			rehydrate.Enter(delegate(StatesInstance smi)
			{
				...
				AccessabilityManager accessabilityManager3 = rehydrate.rehydrator.Get(smi);
				if (accessabilityManager3 != null)
				{
					GameObject worker2 = eater.Get(smi);
					if (accessabilityManager3.CanAccess(worker2))
					{
						accessabilityManager3.Reserve(eater.Get(smi));
					}
					else
					{
			...
			{
				AccessabilityManager accessabilityManager2 = rehydrate.rehydrator.Get(smi);
			...
			}, delegate(StatesInstance smi)
			{
				...
				return !(accessabilityManager == null) && accessabilityManager.CanAccess(eater.Get<Worker>(smi).gameObject);
			...
			eatatmessstation.eat.Enter("AnimOverride", delegate(StatesInstance smi)
			{
				smi.GetComponent<KAnimControllerBase>().AddAnimOverrides(Assets.GetAnim("anim_eat_table_kanim"));
			}).DoEat(ediblechunk, actualfoodunits, null, null).Enter(delegate(StatesInstance smi)
			{
