...
public class PartyEvent : GameplayEvent<PartyEvent.StatesInstance>
{
	...
	public class States : GameplayEventStateMachine<States, StatesInstance, GameplayEventManager, PartyEvent>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			planning.wait_for_input.ToggleNotification((StatesInstance smi) => EventInfoScreen.CreateNotification(GenerateEventPopupData(smi)));
			warmup.ToggleNotification((StatesInstance smi) => EventInfoScreen.CreateNotification(GenerateEventPopupData(smi)));
			...
			canceled.DoNotification((StatesInstance smi) => GameplayEventManager.CreateStandardCancelledNotification(GenerateEventPopupData(smi))).Enter(delegate(StatesInstance smi)
			{
		...
		public override EventInfoData GenerateEventPopupData(StatesInstance smi)
		{
			...
			option.AddPositiveIcon(Assets.GetSprite("overlay_materials"), Effect.CreateFullTooltip(effect, showDuration: true));
			option.tooltip = GAMEPLAY_EVENTS.EVENT_TYPES.PARTY.ACCEPT_OPTION_DESC;
			if (1 == 0)
			{
				option.AddInformationIcon("Cake must be built");
			...
			option2.AddNegativeIcon(Assets.GetSprite("overlay_decor"), Effect.CreateFullTooltip(effect2, showDuration: true));
			eventInfoData.AddDefaultConsiderLaterOption();
			eventInfoData.SetTextParameter("host", planner.Get(smi).GetProperName());
			eventInfoData.SetTextParameter("dupe", guest.Get(smi).GetProperName());
	...
	public PartyEvent()
		: base("Party", 0, 0)
	{
		animFileName = "event_pop_up_assets_kanim";
		title = GAMEPLAY_EVENTS.EVENT_TYPES.PARTY.NAME;
		description = GAMEPLAY_EVENTS.EVENT_TYPES.PARTY.DESCRIPTION;
	}
