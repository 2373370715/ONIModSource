...
public class FoodFightEvent : GameplayEvent<FoodFightEvent.StatesInstance>
{
	public class StatesInstance : GameplayEventStateMachine<States, StatesInstance, GameplayEventManager, FoodFightEvent>.GameplayEventStateMachineInstance
	{
		...
		public void CreateChores(StatesInstance smi)
		{
			...
			for (int i = 0; i < Math.Min(Components.LiveMinionIdentities.Count, buildingsOnFloor.Count); i++)
			{
				...
				FoodFightChore foodFightChore = new FoodFightChore(minionIdentity, locator);
	...
	public class States : GameplayEventStateMachine<States, StatesInstance, GameplayEventManager, FoodFightEvent>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			planning.ToggleNotification((StatesInstance smi) => EventInfoScreen.CreateNotification(GenerateEventPopupData(smi)));
			...
			canceled.DoNotification((StatesInstance smi) => GameplayEventManager.CreateStandardCancelledNotification(GenerateEventPopupData(smi))).Enter(delegate
			{
		...
		public override EventInfoData GenerateEventPopupData(StatesInstance smi)
		{
			...
			eventInfoData.AddOption(GAMEPLAY_EVENTS.EVENT_TYPES.FOOD_FIGHT.ACCEPT_OPTION_NAME).callback = delegate
			{
				smi.GoTo(smi.sm.warmup.wait);
			};
			eventInfoData.AddOption(GAMEPLAY_EVENTS.EVENT_TYPES.FOOD_FIGHT.REJECT_OPTION_NAME).callback = delegate
			{
	...
	public FoodFightEvent()
		: base("FoodFight", 0, 0)
	{
		title = GAMEPLAY_EVENTS.EVENT_TYPES.FOOD_FIGHT.NAME;
		description = GAMEPLAY_EVENTS.EVENT_TYPES.FOOD_FIGHT.DESCRIPTION;
	}
