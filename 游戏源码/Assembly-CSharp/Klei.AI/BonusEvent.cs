...
public class BonusEvent : GameplayEvent<BonusEvent.StatesInstance>
{
	...
	public class States : GameplayEventStateMachine<States, StatesInstance, GameplayEventManager, BonusEvent>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			active.notify.ToggleNotification((StatesInstance smi) => EventInfoScreen.CreateNotification(GenerateEventPopupData(smi)));
		...
		public override EventInfoData GenerateEventPopupData(StatesInstance smi)
		{
			...
			eventInfoData.minions = new GameObject[1] { gameObject };
			...
			foreach (AttributeModifier selfModifier in effect.SelfModifiers)
			{
				...
				string text = string.Format(DUPLICANTS.MODIFIERS.MODIFIER_FORMAT, attribute.Name, selfModifier.GetFormattedString());
		...
		{
			if (!(data is GameplayEventData gameplayEventData))
			{
				return false;
			}
			...
			if (smi.gameplayEvent.buildingTrigger.Count > 0 && !smi.gameplayEvent.buildingTrigger.Contains(gameplayEventData.building.prefabid.PrefabID()))
			{
				return false;
			}
	...
	public BonusEvent(string id, string overrideEffect = null, int numTimesAllowed = 1, bool preSelectMinion = false, int priority = 0)
		: base(id, priority, 0)
	{
		title = Strings.Get("STRINGS.GAMEPLAY_EVENTS.BONUS." + id.ToUpper() + ".NAME");
		description = Strings.Get("STRINGS.GAMEPLAY_EVENTS.BONUS." + id.ToUpper() + ".DESCRIPTION");
		effect = ((overrideEffect != null) ? overrideEffect : id);
		base.numTimesAllowed = numTimesAllowed;
		this.preSelectMinion = preSelectMinion;
		animFileName = id.ToLower() + "_kanim";
