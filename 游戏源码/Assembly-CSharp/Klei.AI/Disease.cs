...
public abstract class Disease : Resource
{
	...
	public Disease(string id, float strength, RangeInfo temperature_range, RangeInfo temperature_half_lives, RangeInfo pressure_range, RangeInfo pressure_half_lives, float radiation_kill_rate, bool statsOnly)
		: base(id)
	{
	...
	public TagGrowthRule GetGrowthRuleForTag(Tag t)
	{
		if (growthRules != null)
		{
			for (int i = 0; i < growthRules.Count; i++)
			{
				if (growthRules[i] is TagGrowthRule tagGrowthRule && tagGrowthRule.tag == t)
				{
	...
	public float GetGrowthRateForTags(HashSet<Tag> tags, bool overpopulated)
	{
		...
		if (growthRules != null)
		{
			for (int i = 0; i < growthRules.Count; i++)
			{
				if (growthRules[i] is TagGrowthRule tagGrowthRule && tags.Contains(tagGrowthRule.tag))
				{
