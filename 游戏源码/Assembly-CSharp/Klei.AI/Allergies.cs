...
public class Allergies : Sickness
{
	...
	public Allergies()
		: base("Allergies", SicknessType.Pathogen, Severity.Minor, 0.00025f, new List<InfectionVector> { InfectionVector.Inhalation }, 60f)
	{
		float value = 0.025f;
		AddSicknessComponent(new CommonSickEffectSickness());
		AddSicknessComponent(new AnimatedSickness(new HashedString[1] { "anim_idle_allergies_kanim" }, Db.Get().Expressions.Pollen));
