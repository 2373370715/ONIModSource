...
public class WarpConduitSender : StateMachineComponent<WarpConduitSender.StatesInstance>, ISecondaryInput
{
	private class ConduitPort
	{
		...
		public bool IsOn()
		{
			...
			{
				return solidConsumer.IsConsuming;
			}
			if (conduitConsumer != null)
			{
				if (conduitConsumer.IsConnected && conduitConsumer.IsSatisfied)
				{
					return conduitConsumer.consumedLastTick;
				}
				return false;
			}
			return false;
	...
	public class States : GameStateMachine<States, StatesInstance, WarpConduitSender>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			on.waiting.QueueAnim("idle").ToggleMainStatusItem(Db.Get().BuildingStatusItems.Normal).Update(delegate(StatesInstance smi, float dt)
			{
				if (smi.master.IsSending())
				{
					smi.GoTo(on.working);
