...
public class CharacterContainer : KScreen, ITelepadDeliverableContainer
{
	...
	private void SetAnimator()
	{
		...
		stats.ApplyOutfit(stats.personality, animController.gameObject);
	...
	private void SetInfoText()
	{
		...
		foreach (KeyValuePair<SkillGroup, float> skillAptitude in stats.skillAptitudes)
		{
			...
			string text6 = "";
			text6 = ((!(skillGroup.choreGroupID != "")) ? string.Format(DUPLICANTS.ROLES.GROUPS.APTITUDE_DESCRIPTION, skillGroup.Name, DUPLICANTSTATS.APTITUDE_BONUS) : string.Format(arg2: Db.Get().ChoreGroups.Get(skillGroup.choreGroupID).description, format: DUPLICANTS.ROLES.GROUPS.APTITUDE_DESCRIPTION_CHOREGROUP, arg0: skillGroup.Name, arg1: DUPLICANTSTATS.APTITUDE_BONUS));
			locText3.GetComponent<ToolTip>().SetSimpleTooltip(text6);
			...
			string text7 = skillAptitude.Key.relevantAttributes[0].Description;
	...
	{
		if (containers.Find((CharacterContainer container) => container != null && container.stats != null && container != this && container.stats.personality.Id == stats.personality.Id && container.stats.IsValid) != null)
		{
			return true;
		}
		if (SaveLoader.Instance != null && DlcManager.IsDlcId(stats.personality.requiredDlcId) && !SaveLoader.Instance.GameInfo.dlcIds.Contains(stats.personality.requiredDlcId))
		{
			return true;
		}
		if (Components.LiveMinionIdentities.Items.Any((MinionIdentity id) => id.personalityResourceId == stats.personality.Id))
		{
			return true;
		}
		return false;
