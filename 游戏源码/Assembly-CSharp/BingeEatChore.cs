...
public class BingeEatChore : Chore<BingeEatChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, BingeEatChore, object>.GameInstance
	{
		...
		public void FindFood()
		{
			...
			{
				if (!item.HasTag(GameTags.Dehydrated) && !(item == null) && !(item == base.sm.ediblesource.Get<Edible>(base.smi)) && !item.isBeingConsumed && !(item.GetComponent<Pickupable>().UnreservedAmount <= 0f) && item.GetComponent<Pickupable>().CouldBePickedUpByMinion(base.gameObject))
				{
