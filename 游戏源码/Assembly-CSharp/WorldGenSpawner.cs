...
public class WorldGenSpawner : KMonoBehaviour
{
	public class Spawnable
	{
		...
		private void Spawn()
		{
			...
			if (gameObject != null && (bool)gameObject)
			{
				...
				gameObject.Trigger(1119167081, spawnInfo);
			}
			FreeResources();
		}
	...
	public IReadOnlyList<Spawnable> GetSpawnables()
	{
		return spawnables;
	}
	...
	protected override void OnSpawn()
	{
		...
		{
			Debug.Assert(SaveLoader.Instance.Cluster != null, "Trying to place templates for an already-loaded save, no worldgen data available");
			DoReveal(SaveLoader.Instance.Cluster);
			PlaceTemplates(SaveLoader.Instance.Cluster);
	...
	public List<Tag> GetUnspawnedWithType<T>(int worldID) where T : KMonoBehaviour
	{
		...
		foreach (Spawnable item in spawnables.FindAll((Spawnable match) => !match.isSpawned && Grid.WorldIdx[match.cell] == worldID && Assets.GetPrefab(match.spawnInfo.id) != null && Assets.GetPrefab(match.spawnInfo.id).GetComponent<T>() != null))
		{
	...
	public List<Spawnable> GetSpawnablesWithTag(bool includeSpawned = false, params Tag[] tags)
	{
		List<Spawnable> list = new List<Spawnable>();
		foreach (Spawnable item in spawnables.FindAll((Spawnable match) => includeSpawned || !match.isSpawned))
		{
			foreach (Tag tag in tags)
			{
				if (item.spawnInfo.id == tag)
				{
					list.Add(item);
					break;
				}
