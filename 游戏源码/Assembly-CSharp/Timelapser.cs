...
public class Timelapser : KMonoBehaviour
{
	...
	private void RefreshRenderTextureSize(object data = null)
	{
		...
		{
			if (bufferRenderTexture != null)
			{
				bufferRenderTexture.DestroyRenderTexture();
			}
			...
			bufferRenderTexture.name = "Timelapser.PreviewScreenshot";
		}
		else if (timelapseUserEnabled)
		{
			...
			bufferRenderTexture.name = "Timelapser.Timelapse";
		}
	}
	...
	private void Update()
	{
		if (screenshotToday)
		{
			if (CycleTimeToScreenshot() <= 0f || GameClock.Instance.GetCycle() == 0)
			{
				...
				else if (!PlayerController.Instance.CanDrag())
				{
	...
	private void SetPostionAndOrtho(int world_id)
	{
		...
		{
			CameraController.Instance.OrthographicSize = world.WorldSize.y / 2;
	...
	private void RenderAndPrint(int world_id)
	{
		...
		CameraController.Instance.OrthographicSize = camSize;
