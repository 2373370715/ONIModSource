...
public class Building : KMonoBehaviour, IGameObjectEffectDescriptor, IUniformGridObject, IApproachable
{
	...
	public string DescFlavour => descriptionFlavour;

	public string DescEffect => Def.Effect;
	...
	public static void CreateBuildingMeltedNotification(GameObject building)
	{
		...
		Notification notification = new Notification(MISC.NOTIFICATIONS.BUILDING_MELTED.NAME, NotificationType.Bad, (List<Notification> notificationList, object data) => string.Concat(MISC.NOTIFICATIONS.BUILDING_MELTED.TOOLTIP, notificationList.ReduceMessages(countNames: false)), "/t• " + notifier.GetProperName(), expires: true, 0f, delegate
		{
