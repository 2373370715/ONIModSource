...
public class ToolParameterMenu : KMonoBehaviour
{
	...
	public void PopulateMenu(Dictionary<string, ToggleState> parameters)
	{
		...
		foreach (KeyValuePair<string, ToggleState> parameter in parameters)
		{
			...
			gameObject.GetComponentInChildren<LocText>().text = Strings.Get("STRINGS.UI.TOOLS.FILTERLAYERS." + parameter.Key + ".NAME");
			ToolTip componentInChildren = gameObject.GetComponentInChildren<ToolTip>();
			if (componentInChildren != null)
			{
				componentInChildren.SetSimpleTooltip(Strings.Get("STRINGS.UI.TOOLS.FILTERLAYERS." + parameter.Key + ".TOOLTIP"));
			}
