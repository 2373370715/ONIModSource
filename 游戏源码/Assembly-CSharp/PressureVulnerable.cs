...
public class PressureVulnerable : StateMachineComponent<PressureVulnerable.StatesInstance>, IGameObjectEffectDescriptor, IWiltCause, ISlicedSim1000ms
{
	...
	{
		float value = base.smi.sm.pressure.Get(base.smi) * 0.7f + GetPressureOverArea(cell) * 0.3f;
		safe_element *= 0.7f;
		if (testAreaElementSafe)
		{
			safe_element += 0.3f;
		}
		displayPressureAmount.value = value;
		bool value2 = safe_atmospheres == null || safe_atmospheres.Count == 0 || safe_element >= 0.06f;
		base.smi.sm.safe_element.Set(value2, base.smi);
		base.smi.sm.pressure.Set(value, base.smi);
	}
