...
public class MinionConfig : IEntityConfig
{
	...
	private void SetupDreams(GameObject prefab)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			entry.anim = dream.anim;
			entry.context = dream.context;
	...
	public static void AddMinionTraits(string name, Modifiers modifiers)
	{
		...
		trait.Add(new AttributeModifier(Db.Get().Attributes.TransitTubeTravelSpeed.Id, 18f, name));
		...
		trait.Add(new AttributeModifier(Db.Get().Amounts.Bladder.deltaAttribute.Id, 1f / 6f, name));
