...
public class AllResourcesScreen : ShowOptimizedKScreen, ISim4000ms, ISim1000ms
{
	...
	public void Init()
	{
		if (!initialized)
		{
			...
			searchInputField.OnValueChangesPaused = delegate
			{
	...
	private void SetRowsActive()
	{
		...
		foreach (KeyValuePair<Tag, ResourceRow> resourceRow in resourceRows)
		{
			if (resourceRow.Value.GameObject.activeSelf != currentlyDisplayedRows[resourceRow.Key])
			{
				...
				if (!currentlyDisplayedRows[resourceRow.Key] && resourceRow.Value.horizontalLayoutGroup.enabled)
				{
