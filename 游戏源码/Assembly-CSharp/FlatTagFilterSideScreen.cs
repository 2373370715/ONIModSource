...
public class FlatTagFilterSideScreen : SideScreenContent
{
	...
	private void Refresh()
	{
		foreach (KeyValuePair<Tag, GameObject> kvp in rows)
		{
			...
			kvp.Value.SetActive(!tagFilterable.displayOnlyDiscoveredTags || DiscoveredResources.Instance.IsDiscovered(kvp.Key));
		}
	}

	public override string GetTitle()
	{
		return tagFilterable.gameObject.GetProperName();
	}
}
