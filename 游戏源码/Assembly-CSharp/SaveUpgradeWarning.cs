...
public class SaveUpgradeWarning : KMonoBehaviour
{
	...
	private IEnumerator TemporaryDisableMeteorShowers(float timeOffDurationInCycles)
	{
		...
		foreach (GameplayEvent resource in Db.Get().GameplayEvents.resources)
		{
			...
			{
				resource.SetSleepTimer(sleepTimer);
			}
		}
		if (!DlcManager.IsPureVanilla())
		{
		...
		{
			GameplayEventManager.Instance.RemoveActiveEvent(item, "Cancelled by SaveUpgradeWarning for player's convenience by providing a window of time without meteors to allow players to adapt to new updates made to relevant buildings");
		}
	}
