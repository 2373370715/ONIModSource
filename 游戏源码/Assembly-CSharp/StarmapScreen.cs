...
public class StarmapScreen : KModalScreen
{
	...
	private string StorageCapacityTooltip(CommandModule command, SpaceDestination dest)
	{
		...
		if (dest != null && flag)
		{
			...
			{
				text = string.Concat(text, UI.STARMAP.LAUNCHCHECKLIST.INSUFFICENT_MASS_TOOLTIP, "\n\n");
			}
	...
	private void FillRocketListPanel()
	{
		...
		foreach (Spacecraft rocket in spacecraft)
		{
			...
			Tuple<string, BreakdownListRow.Status> textForState = GetTextForState(rocket.state, rocket);
			...
			else
			{
				...
				component.AddRow().ShowData(UI.STARMAP.ROCKETSTATUS.TIMEREMAINING, ((rocket.controlStationBuffTimeRemaining <= 0f) ? "" : UI.STARMAP.ROCKETSTATUS.BOOSTED_TIME_MODIFIER.text) + Util.FormatOneDecimalPlace(timeLeft / 600f) + " / " + GameUtil.GetFormattedCycles(duration));
	...
	public static Tuple<string, BreakdownListRow.Status> GetTextForState(Spacecraft.MissionState state, Spacecraft spacecraft)
	{
		return state switch
		{
			...
			Spacecraft.MissionState.Underway => new Tuple<string, BreakdownListRow.Status>((spacecraft.controlStationBuffTimeRemaining <= 0f) ? UI.STARMAP.MISSION_STATUS.UNDERWAY.text : UI.STARMAP.MISSION_STATUS.UNDERWAY_BOOSTED.text, BreakdownListRow.Status.Red), 
	...
	private void UpdateMissionOverlay(LaunchConditionManager lcmToVisualize = null)
	{
		...
		if (lcmToVisualize != null && spacecraft != null)
		{
			...
			if (spacecraft.state != 0 && spacecraftDestination.GetDestinationType().visitable)
			{
				...
				visualizeRocketLabel.text = GetTextForState(spacecraft.state, spacecraft).first;
