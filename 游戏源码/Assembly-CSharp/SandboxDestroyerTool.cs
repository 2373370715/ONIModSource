...
public class SandboxDestroyerTool : BrushTool
{
	...
	protected override void OnPaintCell(int cell, int distFromOrigin)
	{
		...
		foreach (Comet item6 in Components.Meteors.GetItems(Grid.WorldIdx[cell]))
		{
			if (!item6.IsNullOrDestroyed() && Grid.PosToCell(item6) == cell)
			{
				pooledHashSet.Add(item6.gameObject);
			}
		}
		foreach (GameObject item7 in pooledHashSet)
		{
			Util.KDestroyGameObject(item7);
		}
