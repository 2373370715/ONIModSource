...
public class VirtualInputModule : PointerInputModule, IInputHandler
{
	...
	public override void UpdateModule()
	{
		...
		if (inputHandler == null || !inputHandler.UsesController(this, inputManager.GetController(1)))
		{
		...
		m_VirtualCursor.anchoredPosition += steamCursorMovement * m_VirtualCursorSpeed;
	...
	protected void ProcessControllerPress(MouseButtonEventData data, bool leftClick)
	{
		...
		buttonData2.position = m_VirtualCursor.anchoredPosition;
		if (rightFirstClick)
		{
			...
			buttonData2.position = m_VirtualCursor.anchoredPosition;
			DeselectIfSelectionChanged(gameObject3, buttonData2);
			GameObject gameObject4 = ExecuteEvents.ExecuteHierarchy(gameObject3, buttonData2, ExecuteEvents.pointerDownHandler);
			if (gameObject4 == null)
			{
			...
			if (gameObject4 == buttonData2.lastPress)
			{
				...
				{
					int clickCount = buttonData2.clickCount + 1;
					buttonData2.clickCount = clickCount;
				}
				else
				{
			...
			{
				ExecuteEvents.Execute(buttonData2.pointerDrag, buttonData2, ExecuteEvents.initializePotentialDrag);
			}
			rightFirstClick = false;
		}
		...
		{
			buttonData2.button = PointerEventData.InputButton.Right;
			ExecuteEvents.Execute(buttonData2.pointerPress, buttonData2, ExecuteEvents.pointerUpHandler);
			...
			{
				ExecuteEvents.Execute(buttonData2.pointerPress, buttonData2, ExecuteEvents.pointerClickHandler);
			}
			else if (buttonData2.pointerDrag != null && buttonData2.dragging)
			{
				ExecuteEvents.ExecuteHierarchy(gameObject3, buttonData2, ExecuteEvents.dropHandler);
			}
			...
			{
				ExecuteEvents.Execute(buttonData2.pointerDrag, buttonData2, ExecuteEvents.endDragHandler);
			}
			buttonData2.dragging = false;
			buttonData2.pointerDrag = null;
			if (gameObject3 != buttonData2.pointerEnter)
			{
				HandlePointerExitAndEnter(buttonData2, null);
				HandlePointerExitAndEnter(buttonData2, gameObject3);
			}
			rightReleased = false;
		}
	}
