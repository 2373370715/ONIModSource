...
public class SuitLocker : StateMachineComponent<SuitLocker.StatesInstance>
{
	...
	public class ReturnSuitWorkable : Workable
	{
		...
		public Chore.Precondition SuitTypeMatchesLocker = new Chore.Precondition
		{
			...
			fn = delegate(ref Chore.Precondition.Context context, object data)
			{
				...
				return slot.assignable.GetComponent<KPrefabID>().IsAnyPrefabID(suitLocker.OutfitTags) ? true : false;
			}
		};
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			charging.DefaultState(charging.pre).RefreshUserMenuOnEnter().EventTransition(GameHashes.OnStorageChange, empty, (StatesInstance smi) => smi.master.GetStoredOutfit() == null)
				...
				.Enter(delegate(StatesInstance smi)
				{
					...
					component.TryRemoveSymbolOverride("suit_swap");
					if (symbol != null)
					{
						component.AddSymbolOverride("suit_swap", symbol);
					}
				});
	...
	public KPrefabID GetStoredOutfit()
	{
		foreach (GameObject item in GetComponent<Storage>().items)
		{
			if (!(item == null))
			{
				...
				if (!(component == null) && component.IsAnyPrefabID(OutfitTags))
				{
					return component;
				}
			}
	...
	{
		fetchChore = new FetchChore(Db.Get().ChoreTypes.EquipmentFetch, GetComponent<Storage>(), 1f, new HashSet<Tag>(OutfitTags), FetchChore.MatchCriteria.MatchID, Tag.Invalid, new Tag[1] { GameTags.Assigned }, null, run_until_complete: true, null, null, null, Operational.State.None);
