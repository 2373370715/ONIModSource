...
public class DupeGreetingManager : KMonoBehaviour, ISim200ms
{
	...
	private Reactable GetReactable(MinionIdentity minion)
	{
		...
		Emote emote = emotes[Random.Range(0, emotes.Count)];
		SelfEmoteReactable selfEmoteReactable = new SelfEmoteReactable(minion.gameObject, "NavigatorPassingGreeting", Db.Get().ChoreTypes.Emote, 1000f);
		selfEmoteReactable.SetEmote(emote).SetThought(Db.Get().Thoughts.Chatty);
		selfEmoteReactable.RegisterEmoteStepCallbacks("react", BeginReacting, null);
		...
		foreach (GreetingSetup activeSetup in activeSetups)
		{
			if (activeSetup.A.minion == component)
			{
				if (activeSetup.B.minion != null)
				{
					...
					activeSetup.A.minion.Trigger(-594200555, activeSetup.B.minion);
					activeSetup.B.minion.Trigger(-594200555, activeSetup.A.minion);
				}
				break;
			}
