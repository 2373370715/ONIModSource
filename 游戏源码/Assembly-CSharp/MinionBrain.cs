...
public class MinionBrain : Brain
{
	...
	private void RegisterReactEmotePair(string reactable_id, Emote emote, float max_trigger_time)
	{
		if (!(base.gameObject == null))
		{
			...
			if (sMI != null)
			{
				EmoteChore emoteChore = new EmoteChore(base.gameObject.GetComponent<ChoreProvider>(), Db.Get().ChoreTypes.EmoteIdle, emote);
				SelfEmoteReactable selfEmoteReactable = new SelfEmoteReactable(base.gameObject, reactable_id, Db.Get().ChoreTypes.Cough, max_trigger_time);
	...
	private void OnResearchComplete(object data)
	{
		...
		{
			RegisterReactEmotePair("ResearchComplete", Db.Get().Emotes.Minion.ResearchComplete, 3f);
	...
	private void OnUnstableGroundImpact(object data)
	{
		...
		if (!num && !flag)
		{
			RegisterReactEmotePair("UnstableGroundShock", Db.Get().Emotes.Minion.Shock, 1f);
