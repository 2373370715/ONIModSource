...
public class CraftModuleInterface : KMonoBehaviour, ISim4000ms
{
	...
	public float Range
	{
		get
		{
			...
			if (engine != null)
			{
				return BurnableMassRemaining / engine.GetComponent<RocketModuleCluster>().performanceStats.FuelKilogramPerDistance;
			}
			return 0f;
		}
	}
	...
	public void CompleteSelfDestruct(object data = null)
	{
		...
		foreach (RocketModule item in list)
		{
			...
			{
				components[i].DropAll(vent_gas: false, dump_liquid: false, default(Vector3), do_disease_transfer: true, list3);
