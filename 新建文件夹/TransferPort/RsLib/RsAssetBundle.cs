...
public class RsAssetBundle {
    public static AssetBundle LoadAssetBundle(string basePath,
        ...
        if (path.IsNullOrWhiteSpace()) path = Path.Combine(basePath, "assets");
        if (platformSpecific) {
            var platform = Application.platform;
            if (platform != RuntimePlatform.OSXPlayer) {
                if (platform != RuntimePlatform.WindowsPlayer) {
                    if (platform == RuntimePlatform.LinuxPlayer) path = Path.Combine(path, "linux");
                } else
