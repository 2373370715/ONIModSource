...
public class WirelessLogicSenderConfig : WirelessLogicConfig {
    ...
    public override string    defaultName => MYSTRINGS.BUILDINGS.PREFABS.RSWIRELESSLOGICSENDER.DEFAULTNAME;
}
...
public class WirelessLogicReceiverConfig : WirelessLogicConfig {
    ...
    public override string    defaultName => MYSTRINGS.BUILDINGS.PREFABS.RSWIRELESSLOGICRECEIVER.DEFAULTNAME;
}
...
public abstract class WirelessLogicConfig : IBuildingConfig {
    ...
    public override BuildingDef CreateBuildingDef() {
        var buildingDef
            = MyUtils.CreateTransferBuildingDef(id,
                                                anim,
        ...
        if (InOutType == InOutType.Sender)
            buildingDef.LogicInputPorts = new List<LogicPorts.Port> {
                LogicPorts.Port.RibbonInputPort((HashedString)PORT_ID,
                                                new CellOffset(0, 0),
                                                S_BUILDINGS.PREFABS.LOGICDUPLICANTSENSOR.LOGIC_PORT,
                                                S_BUILDINGS.PREFABS.LOGICINTERASTEROIDSENDER.LOGIC_PORT_ACTIVE,
                                                S_BUILDINGS.PREFABS.LOGICINTERASTEROIDSENDER.LOGIC_PORT_INACTIVE,
                                                true)
            };
        else
            buildingDef.LogicOutputPorts = new List<LogicPorts.Port> {
                LogicPorts.Port.RibbonOutputPort((HashedString)PORT_ID,
                                                 new CellOffset(0, 0),
                                                 S_BUILDINGS.PREFABS.LOGICDUPLICANTSENSOR.LOGIC_PORT,
                                                 S_BUILDINGS.PREFABS.LOGICINTERASTEROIDSENDER.LOGIC_PORT_ACTIVE,
                                                 S_BUILDINGS.PREFABS.LOGICINTERASTEROIDSENDER.LOGIC_PORT_INACTIVE,
