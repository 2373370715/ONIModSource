...
public class PortManager : SingleManager<PortManager> {
    ...
    private readonly HashSet<SingleChannelController>                allChannel              = new();
    private readonly Dictionary<TransferPortChannel, PortChannelKey> allPort                 = new();
    ...
    public void Add(TransferPortChannel portChannelItem) {
        if (allPort.ContainsKey(portChannelItem)) throw new Exception("Repeat addition PortChannelItem");

        PortChannelKey channelKey = new(portChannelItem.ChannelName,
                                        portChannelItem.WorldIdAG,
                                        portChannelItem.BuildingType);
        ...
        if (!channels.ContainsKey(nullChannelKey))
            channels.Add(nullChannelKey,
                         CreateSingleChannelController(channelKey.buildingType, "", channelKey.worldId));
    }

    public bool Contains(TransferPortChannel portChannelItem) { return allPort.ContainsKey(portChannelItem); }

    public void Remove(TransferPortChannel portChannelItem) {
        if (!allPort.ContainsKey(portChannelItem)) return;

        var channelKey = allPort[portChannelItem];

        var channels2 = classifyChannels[channelKey.buildingType];
    ...
    public void TriggerChannelChange(TransferPortChannel target) { OnChannelChange?.Invoke(target); }
    ...
    public void BatchChange(SingleChannelController controller, string newName, bool global) {
        if (controller == null || (controller.ChannelName == newName && controller.IsGlobal == global)) return;

        var all = new List<TransferPortChannel>(controller.all);
        foreach (var channel in all) channel.CheckSetChannelNameAndGlobal(newName, global, channel.Priority);
    }
    ...
    public void BatchChangePriority(SingleChannelController controller, int priority) {
        ...
        var all = new List<TransferPortChannel>(controller.all);
        foreach (var channel in all) channel.CheckSetPriority(priority);
    }
    ...
    public ICollection<SingleChannelController> GetChannels(BuildingType buildingType, int worldId, bool sort = false) {
        var list = classifyChannels[buildingType].Values.Where(o => o.WorldIdAG == worldId).ToList();

        if (sort && list.Count > 0) {
            list.Sort();
            if (!string.IsNullOrEmpty(list[0].ChannelName)) {
                var index = list.FindIndex(o => string.IsNullOrEmpty(o.ChannelName));
                if (index != -1) {
                    var temp = list[0];
                    list[0]     = list[index];
                    list[index] = temp;
    ...
    public ICollection<SingleChannelController> GetChannels() { return allChannel; }
    public ICollection<TransferPortChannel>     GetAllPort()  { return allPort.Keys; }

    public SingleChannelController GetChannelController(TransferPortChannel item) {
        PortChannelKey channelKey;
        if (!allPort.TryGetValue(item, out channelKey)) return null;

        var                     buildingType = channelKey.buildingType;
    ...
    public SingleChannelController GetChannelControllerAG(BuildingType buildingType, int worldIdAG, string name) {
        var                     channelKey = new PortChannelKey(name, worldIdAG, buildingType);
        SingleChannelController controller;
        classifyChannels[buildingType].TryGetValue(channelKey, out controller);
        return controller;
    }
    ...
    private void GasConduitUpdate(float dt) {
        foreach (var controller in GetChannels(BuildingType.Gas))
            ((TransferConduitChannel)controller).ConduitUpdate(dt);
    }

    private void LiquidConduitUpdate(float dt) {
        foreach (var controller in GetChannels(BuildingType.Liquid))
            ((TransferConduitChannel)controller).ConduitUpdate(dt);
    }

    private void SolidConduitUpdate(float dt) {
        foreach (var controller in GetChannels(BuildingType.Solid))
            ((TransferConduitChannel)controller).ConduitUpdate(dt);
    }
}