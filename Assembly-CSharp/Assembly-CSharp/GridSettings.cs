...
public class GridSettings : KMonoBehaviour {
    ...
    public static void Reset(int width, int height) {
        ...
        Grid.WidthInMeters        = 1f    * width;
        Grid.HeightInMeters       = 1f    * height;
        ...
        for (var i = 0; i < Grid.ObjectLayers.Length; i++) Grid.ObjectLayers[i] = new Dictionary<int, GameObject>();
        for (var j = 0; j < Grid.CellCount; j++) Grid.Loudness[j]               = 0f;
        ...
        for (var k = 0; k < Grid.CellCount; k++) Grid.WorldIdx[k] = byte.MaxValue;
        Grid.OnReveal = null;
    }
