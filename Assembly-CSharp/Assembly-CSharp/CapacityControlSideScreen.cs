...
public class CapacityControlSideScreen : SideScreenContent {
    [Header("Number Input"), SerializeField]
    ...
    protected override void OnSpawn() {
        ...
        slider.onDrag             += delegate { ReceiveValueFromSlider(slider.value); };
        slider.onPointerDown      += delegate { ReceiveValueFromSlider(slider.value); };
        slider.onMove             += delegate { ReceiveValueFromSlider(slider.value); };
        numberInput.onEndEdit     += delegate { ReceiveValueFromInput(numberInput.currentValue); };
        numberInput.decimalPlaces =  1;
    }

    public override bool IsValidForTarget(GameObject target) {
        return !target.GetComponent<IUserControlledCapacity>().IsNullOrDestroyed() ||
               target.GetSMI<IUserControlledCapacity>() != null;
    }
    ...
    public override void SetTarget(GameObject new_target) {
        ...
        if (target == null) target = new_target.GetSMI<IUserControlledCapacity>();
        if (target == null) {
    ...
    private void ReceiveValueFromSlider(float newValue) { UpdateMaxCapacity(newValue); }
    private void ReceiveValueFromInput(float  newValue) { UpdateMaxCapacity(newValue); }
