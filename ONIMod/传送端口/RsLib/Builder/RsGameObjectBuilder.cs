...
public class RsGameObjectBuilder {
    private static readonly BuilderManager builderManager;
    ...
    static RsGameObjectBuilder() {
        ...
        builderManager.Add(new BuilderFn<Component>(o => {
                                                        o.AppendPrefix();
                                                        o.Append("[").Append(o.target.GetType().FullName);
                                                        o.Append("]");
                                                    }));

        builderManager.Add(new BuilderFn<MonoBehaviour>(o => { o.AppendKeyValue("enabled", o.target.enabled); }));
        ...
        builderManager.Add(new BuilderFn<RectTransform>(o => {
                                                            ...
                                                            o.AppendKeyValue("anchoredPosition",
                                                                             o.target.anchoredPosition);

                                                            o.AppendKeyValue("pivot", o.target.pivot);
                                                            o.AppendKeyValue("rect",  o.target.rect);
                                                        }));

        builderManager.Add(new BuilderFn<Image>(o => {
                                                    o.AppendKeyValue("sprite",
                                                                     o.target.sprite != null
                                                                         ? o.target.sprite.name
                                                                         : null);
                                                }));

        builderManager.Add(new BuilderFn<TMP_Text>(o => {
                                                       o.AppendKeyValue("text", o.target.text);
        ...
        builderManager.Add(new BuilderFn<LayoutElement>(o => {
                                                            var fieldInfos = GetSerializeFields(typeof(LayoutElement));
                                                            foreach (var fieldInfo in fieldInfos)
                                                                o.AppendKeyValue(fieldInfo.Name,
                                                                 fieldInfo.GetValue(o.target));
                                                        }));

        builderManager.Add(new BuilderFn<MultiToggle>(o => {
                                                          o.CheckAddendSpace();
                                                          o.Append("states:{");
                                                          foreach (var toggleState in o.target.states) {
                                                              o.Append("(");
                                                              o.AppendKeyValue("name", toggleState.Name);
                                                              o.AppendKeyValue("sprite",
                                                                               toggleState.sprite != null
                                                                                   ? toggleState.sprite.name
                                                                                   : null);
        ...
        builderManager.Add(new BuilderFn<LocText>(o => {
                                                      o.AppendKeyValue("textStyleSetting",
                                                                       o.target.textStyleSetting != null
                                                                           ? o.target.textStyleSetting.name
                                                                           : null);
        ...
        builderManager.Add(new BuilderFn<SetTextStyleSetting>(o => {
                                                                  var styleSetting
                                                                      = RsField.GetValue(o.target, "style") as
                                                                            TextStyleSetting;

                                                                  o.AppendKeyValue("textStyleSetting",
                                                                   styleSetting != null ? styleSetting.name : null);
                                                              }));

        builderManager.Add(new BuilderFn<RsHierarchyReferences>(o => {
                                                                    var references = o.target.references;
                                                                    if (references != null) {
                                                                        o.CheckAddendSpace();
                                                                        o.Append("references:[");
    ...
    public static string BuildToString(GameObject go) { return builderManager.BuildToString(go, "", true); }

    public static IEnumerable<FieldInfo> GetSerializeFields(Type type) {
        var fields = type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

        var sField = fields.Where(o => (o.IsPublic  && o.GetCustomAttribute<NonSerializedAttribute>() == null) ||
