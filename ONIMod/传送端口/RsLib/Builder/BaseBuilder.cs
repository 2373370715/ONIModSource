...
public class BaseBuilder<T> : IBuilder where T : class {
    public         StringBuilder           builder;
    public         IList<OtherBuilderInfo> otherBuilderInfos;
    public         string                  prefix;
    public         T                       target;
    public virtual bool                    AllowBuild(object obj) { return obj is T; }

    public virtual void Build(object                 target,
                              StringBuilder          builder,
                              string                 prefix,
                              List<OtherBuilderInfo> otherBuilderInfos) {
    ...
    public virtual void CheckAddendSpace() {
        if (LastChar() != ' ' && builder.Length > 0) builder.Append(" ");
    }

    public virtual StringBuilder Append(object text, bool checkAddendSpace = false) {
        if (checkAddendSpace) CheckAddendSpace();
        return builder.Append(text);
    }

    public virtual StringBuilder AppendPrefix()               { return builder.Append(prefix); }
    public virtual void          AppendLine(string text = "") { builder.AppendLine(text); }
    public virtual char          LastChar()                   { return builder[builder.Length - 1]; }
