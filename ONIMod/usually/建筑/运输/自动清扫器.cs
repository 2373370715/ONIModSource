...
public class 自动清扫器 {
    ...
    public static void adjust_radius(ref GameObject go) {
        // go.AddOrGet<LogicOperationalController>();
        go.AddOrGet<SolidTransferArm>().pickupRange = pickupRange;
        var range = go.AddOrGet<SolidTransferArm>().pickupRange;
        Debug.Log("清扫器范围:" + range);
    ...
    public static void adjust_max_pressure(SolidTransferArm __instance) {
        ...
        __instance.GetAttributes().Add(modifier);
    }

    [HarmonyPatch(typeof(SolidTransferArmConfig), "DoPostConfigureUnderConstruction"), HarmonyPostfix]
    public static void Postfix(ref GameObject go) { go.GetComponent<Constructable>().requiredSkillPerk = null; }
